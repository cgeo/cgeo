...
public class CGeoMap extends AbstractMap implements OnDragListener, ViewFactory {
	...
	private OtherCachersOverlay overlayUsers = null;
	...
	private void addMapViewMenuItems(final Menu menu) {		
		MapProviderFactory.addMenuItems(menu, getResources(), settings.getMapSourceId());
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		} else if (MapProviderFactory.isMapSourceMenuItem(item)) {
			
			item.setChecked(true);

			boolean mapRestartRequired = MapProviderFactory.setMapProviderFromMenuItem(settings, mapView, item);
	...
	private class DisplayPointThread extends Thread {
		...
		public void run() {
			...
			if (latitudeIntent != null && longitudeIntent != null) {
				...
				coordinates.add(coord);
				CachesOverlayItemImpl item = settings.getMapFactory().getCacheOverlayItem(coord, null);
