...
class GCUtilsTest {
    ...
    @Test
    public Unit gcCodesBorderCases() {
        //border cases
    ...
    private static Unit testGlCodeRoundtrip(final String logCode, final Long expectedLogId) {
        val gcId: Long = GCUtils.logCodeToLogId(logCode)
        assertThat(gcId).isEqualTo(expectedLogId)
        assertThat(GCUtils.logIdToLogCode(gcId)).isEqualTo(logCode)
    }

    private static Unit testGcCodeRoundtrip(final String gccode, final Long expectedGcId) {
        val gcId: Long = GCUtils.gcCodeToGcId(gccode)
