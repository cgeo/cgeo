...
class DegreeFormulaTest {

    @Test
    public Unit parseDMM() {
    ...
    @Test
    public Unit parseEEFormula() {
        val formula: DegreeFormula = DegreeFormula.compile("E E HC.CIC", true)
    ...
    @Test
    public Unit parseVariousFormulas() {
    ...
    @Test
    public Unit parseFormulaWithWhitespaces() {
    ...
    @Test
    public Unit parseSingleForDebug() {
        assertParse("88. 3.45", null, null, "[88. 3.45?]")
    }

    private Unit assertParse(final String expression, final Function<String, Value> varMap, final Double expectedValue, final String expectedAnnotatedString) {
        val result: ImmutableTriple<Double, CharSequence, Boolean> = DegreeFormula.compile(expression, false).evaluate(varMap)
        val resultValue: Double = result.left
        val annotatedCss: String = TextUtils.annotateSpans(result.middle, s -> {
            if (s is ForegroundColorSpan) {
                val fgs: ForegroundColorSpan = (ForegroundColorSpan) s
                return fgs.getForegroundColor() == Color.RED ? Pair<>("[", "]") : Pair<>("<", ">")
            }
            return Pair<>("{", "}")
        }).replaceAll("]\\[", "").replaceAll("><", "")
        val asString: String = " (Exp:" + expression + ", result:" + resultValue + ", annotatedCss:" + annotatedCss + ")"
    ...
    @Test
    public Unit replaceXWithMultiplicationSign() {
