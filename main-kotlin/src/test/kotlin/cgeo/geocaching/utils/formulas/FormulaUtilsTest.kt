...
class FormulaUtilsTest {
    ...
    private Unit assertSubString(final String expectedResult, final Boolean expectException, final Boolean indexStartsWithZero, final Object ... values) {
        try {
            val result: Value = FormulaUtils.substring(indexStartsWithZero, ValueList.ofPlain(values))
            if (expectException) {
    ...
    public Unit checksum() {
        ...
        //big numbers
        val zeroToNine: String = "1234567890"; //cs = 45
        val bigNumberBase: String = zeroToNine + zeroToNine + zeroToNine + zeroToNine; // cs = 180, ics = 9
    ...
    @Test
    public Unit ifFunction() {
        assertThat(FormulaUtils.ifFunction(ValueList())).isEqualTo(Value.of(0))
        assertThat(FormulaUtils.ifFunction(ValueList().add(Value.of(""), Value.of("a"), Value.of("b")))).isEqualTo(Value.of("b"))
        assertThat(FormulaUtils.ifFunction(ValueList().add(Value.of("true"), Value.of("a"), Value.of("b")))).isEqualTo(Value.of("a"))
        assertThat(FormulaUtils.ifFunction(ValueList().add(Value.of(""), Value.of("a")))).isEqualTo(Value.of(0))
        assertThat(FormulaUtils.ifFunction(ValueList().add(Value.of(0), Value.of("a"), Value.of(1), Value.of("b"), Value.of("c")))).isEqualTo(Value.of("b"))
    }
    ...
    @Test
    public Unit scanForFormulasNoBreakSpaceDoesNotSplitFormulas() {
        assertScanFormula("X\u00A0+\u00A0Y", "X + Y")
    }

    @Test
    public Unit scanForFormulasGC96KBEFindsAllFormulas() {
        assertScanFormula("N 48° (F-H)/2-1.((J-H)*I-2*A+E+9) E 008° (A/G+12).(A+D)*B/2-6*C+49", "(F-H)/2-1", "((J-H)*I-2*A+E+9)", "(A/G+12)", "(A+D)*B/2-6*C+49")
    }

    @Test(timeout = 1000)
    public Unit scanForFormulasGCAHQ78() {
    ...
    @Test(timeout = 1000)
    public Unit scanForFormulasGCAHQ78FullText() {
        //taken from GCAHQ78, preprocessed by c:geo. Related to issue #15173
        val text: String = "„Da musst du durch!“\n" +
            "\n" +
    ...
    @Test
    public Unit scanForFormulasGCAWBHB() {
        val description: String = "Nord = KM + BCO + AFJ + DGN + AF + K + EH + CD + I - L\n" +
    ...
    private Unit assertScanFormula(final String textToScan, final String... expectedFinds) {
        assertScanFormula(textToScan, true, expectedFinds)
    }
    private Unit assertScanFormula(final String textToScan, final Boolean checkExact, final String... expectedFinds) {
        val result: List<String> = FormulaUtils.scanForFormulas(Collections.singleton(textToScan), null)
    ...
    @Test
    public Unit scanForCoordinatesBasics() {
    ...
    @Test
    public Unit scanCoordinatesRemoveDuplicates() {
        assertScanCoordinates("N48 12.345 E10 67.890 something else N48 12.345 E10 67.890", "N48 12.345|E10 67.890")
    }

    @Test
    public Unit scanForCoordinatesGC86KMW() {
        val description: String = "Gründungsjahr c.\n" +
                "\n" +
    ...
    @Test
    public Unit scanForCoordinatesRealLifeCases() {
        //from Issue #12867
    ...
    private Unit assertScanCoordinates(final String textToScan, final String... expectedFindPairs) {
        ...
        if (expectedFindPairs == null || expectedFindPairs.length == 0) {
            ...
            for (String expectedPair : expectedFindPairs) {
                val desc: String = "-ScanCoord(" + idx + ") in '" + textToScan + "'"
