...
class FormulaTest {
    ...
    private static val DOUBLE_OFFSET: Offset<Double> = Offset.offset(0.0000001d)

    private static Double eval(final String expression, final Object... vars) {
        println("EVAL: " + Formula.compile(expression).toDebugString(null, true, true))
    ...
    @Test
    public Unit bigIntegers() {
        //Long value
        val longMax: String = String.valueOf(Long.MAX_VALUE); // this should be 9223372036854775807
        val longMaxCs: Int = 88; // manually calculated checksum of Long.MAX_VALUE
        ...
        //Beyond Long Value
        val longMaxBeyond: String = longMax + "1"
        val longMaxBeyondValue: Value = Formula.evaluate(longMaxBeyond)
        assertThat(longMaxBeyondValue.isInteger()).isTrue()
        assertThat(longMaxBeyondValue.getAsInteger()).isEqualTo(BigInteger(longMaxBeyond)); // if too Long for parsing -> return 0
    ...
    @Test
    public Unit singleTestForDebug() {
        assertThat(eval("(3 + 5 )")).isEqualTo(8d)
    }
    ...
    @Test
    public Unit variableCalculations() {
    ...
    @Test
    public Unit variablesWithDecimals() {
    ...
    private Unit assertRangeFormula(final String formula, final Function<String, Value> varMap, final Object... expectedResults) {
        val f: Formula = Formula.compile(formula)
        assertThat(f.getRangeIndexSize()).isEqualTo(expectedResults.length)
        val results: List<Value> = ArrayList<>()
        for (Int i = 0; i < f.getRangeIndexSize(); i++) {
            results.add(f.evaluate(varMap, i))
        }
        val expectedResultList: List<Value> = ArrayList<>()
        for (Object o : expectedResults) {
            expectedResultList.add(o is Value ? (Value) o : Value.of(o))
        }
    ...
    @Test
    public Unit stringsAndStartEndTokens() {
    ...
    @Test
    public Unit operatorPrecedenceAndEvaluationOrder() {
    ...
    @Test
    public Unit unknownOperator() {
        assertThatThrownBy(() -> eval("2 & 5"))
                .isInstanceOf(FormulaException.class).hasMessageContaining(UNEXPECTED_TOKEN.name())
    }

    @Test
    public Unit unknownVariables() {
    ...
    private Unit assertFormulaPart(final String formulaString, final Int startPos, final String stopChars, final String expectedValue, final String expectedFormPart) {
        val f: Formula = Formula.compile(formulaString, startPos, KeyableCharSet.createFor(stopChars))
        assertThat(f.evaluate().getAsString()).isEqualTo(expectedValue)
        assertThat(f.getExpression()).isEqualTo(expectedFormPart)
    }

    @Test
    public Unit incorrectFunctionCall() {
    ...
    @Test
    public Unit evaluateToCharSequence() {
     ...
     */
    private Unit assertCharSequence(final CharSequence cs, final String expectedString) {
        val annotated: String = TextUtils.annotateSpans(cs, s -> {
            if (s is ForegroundColorSpan && ((ForegroundColorSpan) s).getForegroundColor() == Color.RED) {
                return Pair<>("[", "]")
            }
            return Pair<>("", "")
        })
    ...
    @Test
    public Unit efficientFormulaNodeTrees() {
    ...
    @Test
    public Unit checkFormatter() {
        val c: Char = 'E'
        val d: Double = 9.1324567934d
        assertThat(String.format(Locale.US, "%c%09.5f°", c, d)).isEqualTo("E009.13246°")
    }

    @Test
    public Unit userComments() {
