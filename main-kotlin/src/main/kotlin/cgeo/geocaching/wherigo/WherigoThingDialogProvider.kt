...
class WherigoThingDialogProvider : IWherigoDialogProvider {

    private static val openThingDialogs: Set<Dialog> = Collections.synchronizedSet(HashSet<>())

    private final EventTable eventTable

    private enum class ThingAction {
    ...
    public static Unit closeAllThingDialogs() {
    ...
    override     public Dialog createAndShowDialog(final Activity activity, final IWherigoDialogControl control) {
        ...
        val dialog: AlertDialog = WherigoViewUtils.createFullscreenDialog(activity, "--", binding.getRoot())
    ...
    override     public Boolean canRefresh(final IWherigoDialogProvider otherDialog) {
        //if a "thing" dialog is open and should be opened again for the same thing
        //-> then just refresh, do not close the old dialog just to open a one for same thing
        //-> this enables Wherigo Cartridges to have media animations, see #17439
        return otherDialog is WherigoThingDialogProvider &&
            ((WherigoThingDialogProvider) otherDialog).eventTable == this.eventTable
    }

    private Unit refreshGui(final Activity activity, final IWherigoDialogControl control, final WherigoThingDetailsBinding binding) {
        ...
        binding.headerInformation.setVisibility(eventTable is Zone ? View.VISIBLE : View.GONE)
        if (eventTable is Zone) {
    ...
    private Unit refreshActionList(final Activity activity, final IWherigoDialogControl control, final WherigoThingDetailsBinding binding) {

        //"actions" will be filled with instance of both "Action" and "ThingAction"
        val actions: List<Object> = ArrayList<>()
        if (eventTable is Thing) {
        ...
        WherigoViewUtils.setViewActions(actions, binding.dialogActionlist, 1,
                ...
                item -> {
                    ...
                    val thingAction: ThingAction = (ThingAction) item
                    switch (thingAction) {
                        case DISPLAY_ON_MAP:
                            control.dismiss()
                        ...
                        case LOCATE_ON_CENTER:
                            control.dismiss()
                            val center: Geopoint = WherigoUtils.getZoneCenter((Zone) eventTable)
