...
class WherigoCartridgeDialogProvider : IWherigoDialogProvider {
    ...
    override     public Dialog createAndShowDialog(final Activity activity, final IWherigoDialogControl control) {
        val binding: WherigoCartridgeDetailsBinding = WherigoCartridgeDetailsBinding.inflate(LayoutInflater.from(activity))
        val dialog: AlertDialog = WherigoViewUtils.createFullscreenDialog(activity, cartridgeFile.name, binding.getRoot())

        val saveGames: List<WherigoSavegameInfo> = WherigoSavegameInfo.getLoadableSavegames(cartridgeInfo.getFileInfo())
    ...
    private Unit refreshGui(final WherigoCartridgeDetailsBinding binding) {
        val link: String = WherigoUtils.getWherigoDetailsUrl(cartridgeInfo.getCGuid())
        TextParam.text("**CGUID:** " + cartridgeInfo.getCGuid() + "  \n" +
    ...
    private Unit performActionAfterGameEnded(final CartridgeAction action, final Activity activity, final List<WherigoSavegameInfo> saveGames) {
    ...
    private Unit playGame(final Activity activity) {

        val loadGameList: List<WherigoSavegameInfo> = WherigoSavegameInfo.getLoadableSavegames(this.cartridgeInfo.getFileInfo())
        if (loadGameList.size() == 1 && loadGameList.get(0).isNewGame()) {
            //no savegames present --> just start a game
