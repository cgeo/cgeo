...
class StringFilter {
    ...
    private static val FLAG_MATCHCASE: String = "match_case"

    enum class class StringFilterType {
    ...
    private var filterType: StringFilterType = getDefaultFilterType()
    ...
    public Unit setFilterType(final StringFilterType filterType) {
    ...
    public Unit setTextValue(final String textValue) {
    ...
    public Unit setMatchCase(final Boolean matchCase) {
    ...
    public Unit setJsonConfig(final JsonNode node) {
        if (node != null) {
            ...
            val filterType: String = JsonUtils.getText(node, "type", null)
    ...
    public Boolean matches(final String value) {
        ...
        val valueIsEmpty: Boolean = StringUtils.isEmpty(value)
        ...
        val matchGcValue: String = this.matchCase ? value : StringUtils.lowerCase(value)
        val matchTextValue: String = this.matchCase ? this.textValue : StringUtils.lowerCase(this.textValue)
    ...
    public Unit addToSql(final SqlBuilder sqlBuilder, final String columnExpression) {
        if (!isFilled()) {
    ...
    public Unit addToSqlForSubquery(final SqlBuilder sqlBuilder, final String subQuery, final Boolean subQueryContainsWhere, final String subQueryColumn) {
        if (!isFilled()) {
            sqlBuilder.addWhereTrue()
        } else {
            switch (filterType) {
                ...
                case IS_PRESENT:
                    ...
                    break
                default:
                    val sb: StringBuilder = StringBuilder("EXISTS(" + subQuery + (subQueryContainsWhere ? " AND " : " WHERE "))
