...
class StringFilterViewHolder<F : StringGeocacheFilter()> : BaseFilterViewHolder()<F> {

    private val selectSpinner: TextSpinner<StringFilter.StringFilterType> = TextSpinner<>()
    ...
    public StringFilterViewHolder(final Func1<String, String[]> autoTextCompleteFunction) {
        this.autoTextCompleteFunction = autoTextCompleteFunction
    }

    override     public Unit setViewFromFilter(final F filter) {
        val stringFilter: StringFilter = filter.getStringFilter()
    ...
    override     public View createView() {
        val view: View = inflateLayout(R.layout.cache_filter_generic_string)
        ...
        selectSpinner
                ...
                .setChangeListener(sft -> {
                    val textEnabled: Boolean = sft != StringFilter.StringFilterType.IS_NOT_PRESENT && sft != StringFilter.StringFilterType.IS_PRESENT
        ...
        if (this.autoTextCompleteFunction != null) {
            val actv: AutoCompleteTextView = (AutoCompleteTextView) binding.searchtext.getEditText()
            val adapter: AutoCompleteAdapter = AutoCompleteAdapter(getActivity(), android.R.layout.simple_dropdown_item_1line, this.autoTextCompleteFunction)
