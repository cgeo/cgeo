...
class FilterViewHolderCreator {
    ...
    private static IFilterViewHolder<?> createFor(final GeocacheFilterType type, final Activity activity, final IGeocacheFilter filter) {
        ...
        switch (type) {
            ...
            case PERSONAL_NOTE:
            case OFFLINE_LOG:
                result = StringFilterViewHolder<>()
                break
            case INVENTORY_COUNT:
                result = NumberCountFilterViewHolder(0, 100)
                break
            case TYPE:
                result = CheckboxFilterViewHolder<>(
                ...
                break
            case SIZE:
                result = ChipChoiceFilterViewHolder<>(
            ...
            case DIFFICULTY_TERRAIN:
                result = DifficultyAndTerrainFilterViewHolder()
                break
            case DIFFICULTY_TERRAIN_MATRIX:
                result = DifficultyTerrainMatrixFilterViewHolder()
                break
            case STATUS:
                result = StatusFilterViewHolder()
                break
            case INDIVIDUAL_ROUTE:
                result = BooleanFilterViewHolder(R.string.cache_filter_boolean_individualroute_contained, ImageParam.id(R.drawable.map_quick_route))
                break
            case ATTRIBUTES:
                result = AttributesFilterViewHolder()
                break
            case FAVORITES:
                result = FavoritesFilterViewHolder()
                break
            case DISTANCE:
                result = DistanceFilterViewHolder()
                break
            case HIDDEN:
                result = DateRangeFilterViewHolder<HiddenGeocacheFilter>(true,
            ...
            case LAST_FOUND:
                ...
                result = DateRangeFilterViewHolder<LastFoundGeocacheFilter>(true,
                        values,
                        LocalizationUtils.getStringArray(R.array.cache_filter_hidden_since_stored_values_label),
                        LocalizationUtils.getStringArray(R.array.cache_filter_hidden_since_stored_values_label_short))
                break
            case LOGS_COUNT:
                result = LogsCountFilterViewHolder()
                break
            case LOG_ENTRY:
                result = LogEntryFilterViewHolder()
                break
            case LOCATION:
                result = StringFilterViewHolder<>(DataStore::getSuggestionsLocation)
                break
            case STORED_LISTS:
                result = createStoredListFilterViewHolder()
                break
            case NAMED_FILTER:
                result = NamedFilterFilterViewHolder()
                break
            case ORIGIN:
                result = CheckboxFilterViewHolder<>(
                        ...
                        HashSet<>(ConnectorFactory.getActiveConnectors()))
                break
            case STORED_SINCE:
                result = DateRangeFilterViewHolder<HiddenGeocacheFilter>(true,
            ...
            case LOGICAL_FILTER_GROUP:
                result = LogicalFilterViewHolder()
                break
            default:
                result = null
                break
    ...
    private static <T : IGeocacheFilter()> Unit fillViewFrom(final IFilterViewHolder<T> viewHolder, final IGeocacheFilter filter) {
        if (viewHolder != null && filter != null) {
            viewHolder.setViewFromFilter((T) filter)
        }
    }

    public static <T : IGeocacheFilter()> IGeocacheFilter createFrom(final IFilterViewHolder<T> holder) {
        val filter: IGeocacheFilter = holder.createFilterFromView()
        if (filter == null || filter.getType() == null) {
            throw IllegalStateException("ViewHolder did not create valid filter: " + holder.getClass().getName())
        }
        return filter
    }
    ...
    public static Unit clearListInfo() {
        listInfoFilled = false
        listInfoFilteredList = null
        listInfoIsComplete = false
    }

    public static Unit setListInfo(final Collection<Geocache> filteredList, final Boolean isComplete) {
    ...
    private static IFilterViewHolder<?> create1to5ItemRangeSelectorViewHolder() {
        final Float[] range = Float[]{1.0f, 1.5f, 2.0f, 2.5f, 3.0f, 3.5f, 4.0f, 4.5f, 5.0f}
        return ItemRangeSelectorViewHolder<>(
                ValueGroupFilterAccessor<Float, NumberRangeGeocacheFilter<Float>>()
    ...
    private static IFilterViewHolder<?> createStoredListFilterViewHolder() {
        ...
        val allLists: List<StoredList> = DataStore.getLists()
        val allListsById: Map<Integer, StoredList> = HashMap<>()
        for (StoredList list : allLists) {
            allListsById.put(list.id, list)
        }

        val vgfa: ValueGroupFilterAccessor<StoredList, StoredListGeocacheFilter> =
                ValueGroupFilterAccessor<StoredList, StoredListGeocacheFilter>()
        ...
        return StoredListsFilterViewHolder<>(vgfa, 1, Collections.emptySet())
    }
}
