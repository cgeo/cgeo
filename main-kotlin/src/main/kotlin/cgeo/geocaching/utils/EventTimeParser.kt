...
class EventTimeParser {
     ...
     */
    private static val EVENT_DATE_FORMAT: SynchronizedDateFormat = SynchronizedDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.US); // 2010-04-20T07:00:00

    private static val EVENT_TIME_PATTERNS: List<Pattern> = ArrayList<>()

    private static val UNKNOWN: Int = -1
    ...
    public static Int guessEventTimeMinutes(final String searchText) {
        val eventTimeStandardFormat: Int = getEventTimeStandardFormat(searchText)
        ...
        for (final Pattern pattern : getEventTimePatterns()) {
            val matcher: MatcherWrapper = MatcherWrapper(pattern, searchText)
            while (matcher.find()) {
                try {
                    val hours: Int = Integer.parseInt(matcher.group(1))
                    Int minutes = 0
    ...
    public static Int[] getEventTimesFromGcShortDesc(final String searchText) {
        final Int[] times = Int[] { -1, -1 }
        val gcShortDescPattern: Pattern = Pattern.compile("^<b>\\d{2} [A-Za-z]+ \\d{4}, (\\d{2}):(\\d{2}) - (\\d{2}):(\\d{2})</b>$")
        val matcher: MatcherWrapper = MatcherWrapper(gcShortDescPattern, searchText)
        if (matcher.matches()) {
    ...
    private static Int getEventTimeStandardFormat(final String searchText) {
        val eventStandardText: String = StringUtils.substringBetween(searchText, "dtstart_", "\"")
        if (eventStandardText != null) {
            try {
                val eventStandardDate: Date = EVENT_DATE_FORMAT.parse(eventStandardText)
    ...
    private static List<Pattern> getEventTimePatterns() {
        if (EVENT_TIME_PATTERNS.isEmpty()) {
            val hourLocalized: String = CgeoApplication.getInstance().getString(R.string.cache_time_full_hours)
