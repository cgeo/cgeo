 ...
 */
enum class class FormulaFunction {

    SQRT(String[]{"sqrt"}, FunctionGroup.SIMPLE_NUMERIC, R.string.formula_function_sqrt, "Square Root", null, 0,
            singleValueDoubleFunction(Math::sqrt)),
    SIN(String[]{"sin"}, FunctionGroup.SIMPLE_NUMERIC, R.string.formula_function_sin, "Sinus", null, 0,
            singleValueDoubleFunction(p -> Math.sin(Math.toRadians(p)))),
    COS(String[]{"cos"}, FunctionGroup.SIMPLE_NUMERIC, R.string.formula_function_cos, "Cosinus", null, 0,
            singleValueDoubleFunction(p -> Math.cos(Math.toRadians(p)))),
    TAN(String[]{"tan"}, FunctionGroup.SIMPLE_NUMERIC, R.string.formula_function_tan, "Tangens", null, 0,
            singleValueDoubleFunction(p -> Math.tan(Math.toRadians(p)))),
    ABS(String[]{"abs"}, FunctionGroup.SIMPLE_NUMERIC, R.string.formula_function_abs, "Absolute Value", null, 0,
            singleValueNumericFunction(v -> v.getAsDecimal().abs())),
    ROUND(String[]{"round", "rd"}, FunctionGroup.SIMPLE_NUMERIC, R.string.formula_function_round, "Round", null, 0,
            valueList -> FormulaUtils.truncRound(valueList, false)),
    TRUNC(String[]{"trunc", "tr", "floor", "fl"}, FunctionGroup.SIMPLE_NUMERIC, R.string.formula_function_trunc, "Trunc", null, 0,
            valueList -> FormulaUtils.truncRound(valueList, true)),
    IF(String[]{"if"}, FunctionGroup.COMPLEX_NUMERIC, R.string.formula_function_if, "If", null, 0,
            FormulaUtils::ifFunction),
    LENGTH(String[]{"length", "len"}, FunctionGroup.SIMPLE_STRING, R.string.formula_function_length, "String Length", "''", 1,
            singleValueStringFunction(String::length)),
    SUBSTRING(String[]{"substring", "substr", "sub"}, FunctionGroup.SIMPLE_STRING, R.string.formula_function_substring, "Substring", "'';0;1", 1,
            vl -> FormulaUtils.substring(false, vl)),
    CHARS(String[]{"chars", "ch"}, FunctionGroup.SIMPLE_STRING, R.string.formula_function_chars, "Select Chars", "'';1;2", 1,
        FormulaUtils::selectChars),

    ROT13(String[]{"rot13"}, FunctionGroup.COMPLEX_STRING, R.string.formula_function_rot13, "Rotate characters by 13", "''", 1,
            valueList -> FormulaUtils.rot(valueList, true)),
    ROT(String[]{"rot"}, FunctionGroup.COMPLEX_STRING, R.string.formula_function_rot, "Rotate characters by x", "'';13", 1,
            valueList -> FormulaUtils.rot(valueList, false)),
    CHECKSUM(String[]{"checksum", "cs"}, FunctionGroup.COMPLEX_NUMERIC, R.string.formula_function_checksum, "Checksum", null, 0,
            valueList -> FormulaUtils.checksum(valueList, false)),
    ICHECKSUM(String[]{"ichecksum", "ics"}, FunctionGroup.COMPLEX_NUMERIC, R.string.formula_function_ichecksum, "Iterative Checksum", null, 0,
            valueList -> FormulaUtils.checksum(valueList, true)),
    LETTERVALUE(String[]{"lettervalue", "lv", "wordvalue", "wv", "bww"}, FunctionGroup.COMPLEX_STRING, R.string.formula_function_lettervalue, "Letter Value", "''", 1,
            singleValueStringFunction(FormulaUtils::letterValue)),
    ROMAN(String[]{"roman"}, FunctionGroup.COMPLEX_STRING, R.string.formula_function_roman, "Roman", "''", 1,
            singleValueStringFunction(FormulaUtils::roman)),
    VANITY(String[]{"vanity", "vanitycode", "vc"}, FunctionGroup.COMPLEX_STRING, R.string.formula_function_vanity, "Vanity", "''", 1,
            singleValueStringFunction(FormulaUtils::vanity))

    enum class class FunctionGroup {
    ...
    private static val FUNC_BY_NAME: Map<String, FormulaFunction> = HashMap<>()
    ...
    public Value execute(final ValueList params) throws FormulaException {
        try {
            val result: Object = function.apply(params)
            return result is Value ? (Value) result : Value.of(result)
        } catch (FormulaException fe) {
            throw fe
        } catch (RuntimeException re) {
            throw FormulaException(re, null, OTHER, re.getMessage())
        }
    }
    ...
    public static List<FormulaFunction> valuesAsUserDisplaySortedList() {
        val list: List<FormulaFunction> = ArrayList<>(Arrays.asList(values()))
    ...
    private static Function<ValueList, Object> singleValueNumericFunction(final Function<Value, Number> numericFunction) {
        return params -> {
            ...
            try {
                ...
                throw FormulaException(FormulaException.ErrorType.NUMERIC_OVERFLOW)
            }
        }
    }
