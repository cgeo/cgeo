...
class LocalizationUtils {
    ...
    private static val APPLICATION_CONTEXT: Context =
            CgeoApplication.getInstance() == null ? null : CgeoApplication.getInstance().getApplicationContext()

    private static val LANGUAGE_TO_MAIN_COUNTRY: Map<String, String> = HashMap<>()
    private static final Map<String, Set<String>> LANGUAGE_TO_COUNTRIES = HashMap<>()
    ...
    public static String[] getStringArray(@ArrayRes final Int arrayId, final String... fallback) {
        if (APPLICATION_CONTEXT == null) {
            return fallback == null ? String[0] : fallback
        }
    ...
    public static Int[] getIntArray(@ArrayRes final Int arrayId, final Int... fallback) {
        if (APPLICATION_CONTEXT == null) {
            return fallback == null ? Int[0] : fallback
        }
    ...
    public static ImmutablePair<String, String> getMultiPurposeString(@StringRes final Int messageId, final String fallback, final Object... params) {
        ...
        final Object[] paramsForLog = Object[params.length]
        final Object[] paramsForUser = Object[params.length]
        ...
        for (Int i = 0; i < params.length; i++) {
            ...
            try {
                ...
                } else if (params[i] is PersistableFolder) {
        ...
        return ImmutablePair<>(getStringWithFallback(messageId, fallback, paramsForUser), getStringWithFallback(messageId, fallback, paramsForLog))
    }

    public static String getEnglishString(final Context context, @StringRes final Int resId) {
        val configuration: Configuration = getEnglishConfiguration(context)
    ...
    private static Configuration getEnglishConfiguration(final Context context) {
        val configuration: Configuration = Configuration(context.getResources().getConfiguration())
        configuration.setLocale(Locale("en"))
        return configuration
    }

    private static Set<String> getCountriesForLanguage(final String language) {
        val set: Set<String> = LANGUAGE_TO_COUNTRIES.get(language)
    ...
    private static String getMainCountryForLanguage(final String language) {
        ...
        val candidate: String = LANGUAGE_TO_MAIN_COUNTRY.get(language.toLowerCase(Locale.ROOT))
        if (candidate != null) {
            return candidate.toUpperCase(Locale.ROOT)
        }
        val countries: Set<String> = getCountriesForLanguage(language)
        if (countries.size() == 1) {
    ...
    public static String getLocaleDisplayName(final String languageTag, final Boolean doShort, final Boolean addFlag) {
        return getLocaleDisplayName(languageTag == null ? null : Locale(languageTag), doShort, addFlag)
    }

    public static String getLocaleDisplayName(final Locale locale, final Boolean doShort, final Boolean addFlag) {
        //flag
        val flag: String = addFlag ? getLocaleDisplayFlag(locale) + " " : ""
        if (locale == null) {
            return flag + "--"
        }

        //name
        val nameInAppLocale: String = locale.getDisplayName(Settings.getApplicationLocale())
        val nameInLocaleLocale: String = locale.getDisplayName(locale)
        val name: String = nameInAppLocale + (doShort || nameInAppLocale == (nameInLocaleLocale) ? "" : "/" + nameInLocaleLocale)
