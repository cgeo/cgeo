...
class InstallWizardActivity : AbstractActivity() {

    public static val BUNDLE_MODE: String = "wizardmode"
    private static val BUNDLE_STEP: String = "step"
    private static val BUNDLE_CSAH: String = "csah"
    private static val BUNDLE_BACKUPUTILS: String = "backuputils"

    private static val DO_LEGACY_WRITE_STORAGE: Boolean = android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.Q; // < SDK29

    enum class class WizardMode {
    ...
    private var mode: WizardMode = WizardMode.WIZARDMODE_DEFAULT
    private var step: WizardStep = WizardStep.WIZARD_START
    private var forceSkipButton: Boolean = false
    private var contentStorageActivityHelper: ContentStorageActivityHelper = null
    private BackupUtils backupUtils

    private static val REQUEST_CODE_WIZARD_GC: Int = 0x7167

    private val askLocationPermissionAction: PermissionAction<Void> = PermissionAction.register(this, PermissionContext.LOCATION, b -> gotoNext())
    private val askLegacyStoragePermissionAction: PermissionAction<Void> = PermissionAction.register(this, PermissionContext.LEGACY_WRITE_EXTERNAL_STORAGE, b -> gotoNext())
    private val askNotificationsPermissionAction: PermissionAction<Void> = PermissionAction.register(this, PermissionContext.NOTIFICATIONS, b -> gotoNext())

    // dialog elements
    private var title: TextView = null
    private var text: TextView = null

    private var button1Info: TextView = null
    private var button1: Button = null
    private var button2Info: TextView = null
    private var button2: Button = null
    private var button3Info: TextView = null
    private var button3: Button = null

    private var prev: Button = null
    private var skip: Button = null
    private var next: Button = null
    private var nextOutlined: Button = null
    ...
    override     public Unit onCreate(final Bundle savedInstanceState) {
        ...
        backupUtils = BackupUtils(this, savedInstanceState == null ? null : savedInstanceState.getBundle(BUNDLE_BACKUPUTILS))
        ...
        val binding: InstallWizardBinding = InstallWizardBinding.inflate(getLayoutInflater())
        ...
        this.contentStorageActivityHelper = ContentStorageActivityHelper(this, savedInstanceState == null ? null : savedInstanceState.getBundle(BUNDLE_CSAH))
    ...
    private Unit updateDialog() {
        ...
        switch (step) {
            ...
            case WIZARD_END: {
                ...
                val platforms: StringBuilder = StringBuilder()
    ...
    private Unit setButtonToDone() {
        next.setText(R.string.done)
        next.setVisibility(View.VISIBLE)
        nextOutlined.setVisibility(View.GONE)
    }

    private Unit setNavigation(final Runnable listenerPrev, final Int prevLabelRes, final Runnable listenerSkip, final Int skipLabelRes, final Runnable listenerNext, final Int nextLabelRes) {
        ...
        val useNextOutlinedButton: Boolean = nextLabelRes == R.string.skip
    ...
    private Unit setSkip(final Runnable listenerSkip, final Int skipLabelRes) {
    ...
    private Unit setButton(final Button button, final Int labelResId, final View.OnClickListener listener, final TextView buttonInfo, final Int infoResId) {
        if (button != null) {
            if (listener == null) {
    ...
    private Unit gotoNext() {
        val max: Int = WizardStep.values().length - 1
        if (step.ordinal() < max) {
            step = WizardStep.values()[step.ordinal() + 1]
            if (stepCanBeSkipped()) {
                gotoNext()
    ...
    public static Boolean isConfigurationOk() {
        val isPlatformConfigured: Boolean = ConnectorFactory.getActiveConnectorsWithValidCredentials().length > 0
    ...
    private Unit requestNotifications() {
        setSkip(this::gotoNext, 0)
    ...
    private Unit setFolderInfo(final PersistableFolder folder, @StringRes final Int info, final Boolean addSelectOrCreateInfo) {
        title.setText(String.format(getString(R.string.wizard_permissions_folder_title), getString(folder.getNameKeyId())))
        val temp: String = getString(info) + (addSelectOrCreateInfo ? " " + getString(R.string.wizard_select_or_create) : "")
        text.setText(temp)
    }

    private Unit requestBasefolder() {
    ...
    private Unit onReturnFromFolderMigration(final Boolean resultOk) {
        if (resultOk) {
            gotoNext()
        } else {
    ...
    private Unit requestMapthemefolder() {
        forceSkipButton = false
    ...
    private Unit requestBroutertilesfolder() {
        forceSkipButton = false
    ...
    private Unit prepareFolderDefaultValues() {
    ...
    private Unit authorizeGC() {
        val checkIntent: Intent = Intent(this, GCAuthorizationActivity.class)
        val credentials: Credentials = GCConnector.getInstance().getCredentials()
