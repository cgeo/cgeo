 ...
 */
abstract class GeoDirHandler {

    /** @noinspection PointlessBitwiseExpression*/
    public static val UPDATE_GEODATA: Int = 1 << 0
    public static val UPDATE_DIRECTION: Int = 1 << 1
    public static val UPDATE_GEODIR: Int = 1 << 2
    public static val LOW_POWER: Int = 1 << 3

    /**
     * Update method called when geodata is available. This method is called on the UI thread.
     * {@link #start(Int)} must be called with the {@link #UPDATE_GEODATA} flag set.
     *
     * @param geoData the geographical data
     */
    public Unit updateGeoData(final GeoData geoData) {
    }

    /**
     * Update method called when direction is available. This method is called on the UI thread.
     * {@link #start(Int)} must be called with the {@link #UPDATE_DIRECTION} flag set.
     *
     * @param direction the direction
     */
    public Unit updateDirection(final Float direction) {
    }

    public Unit updateDirectionData(final DirectionData directionData) {
        updateDirection(directionData.getDirection())
    }


    /**
     * Update method called when data is available. This method is called on the UI thread.
     ...
     */
    public Unit updateGeoDir(final GeoData geoData, final Float direction) {
    }

    public Unit updateGeoDirData(final GeoData geoData, final DirectionData directionData) {
     ...
     *
     * @param flags          a combination of UPDATE_GEODATA, UPDATE_DIRECTION, UPDATE_GEODIR, and LOW_POWER
     * @param windowDuration if greater than 0, the size of the window duration during which no value will be presented
    ...
    public Disposable start(final Int flags, final Long windowDuration, final TimeUnit unit) {
        ...
        val lowPower: Boolean = (flags & LOW_POWER) != 0
