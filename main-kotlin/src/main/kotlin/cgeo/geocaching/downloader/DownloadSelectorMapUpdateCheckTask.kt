...
class DownloadSelectorMapUpdateCheckTask : AsyncTaskWithProgressText()<Void, List<Download>> {
    ...
    override     protected List<Download> doInBackgroundInternal(final Void[] none) {
        ...
        result.add(Download(activity.getString(R.string.downloadmap_title), current.mapBase, true, "", "", current.offlineMapType, AbstractDownloader.ICONRES_FOLDER))
        for (CompanionFileUtils.DownloadedFileData installedOfflineMap : installedOfflineMaps) {
            val offlineMap: Download = checkForUpdate(installedOfflineMap)
    ...
    private Download checkForUpdate(final CompanionFileUtils.DownloadedFileData offlineMapData) {
        ...
        String page = ""
        try {
            val response: Response = Network.getRequest(downloader.getUpdatePageUrl(offlineMapData.remotePage), params).blockingGet()
