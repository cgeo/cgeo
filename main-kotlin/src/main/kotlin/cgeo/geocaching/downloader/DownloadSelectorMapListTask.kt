...
class DownloadSelectorMapListTask : AsyncTaskWithProgressText()<Void, List<Download>> {
    ...
    override     protected List<Download> doInBackgroundInternal(final Void[] none) {
        ...
        if (current.companionType != null) {
            if (lastCompanionType == null || !lastCompanionType == (current.companionType) || lastCompanionList.isEmpty()) {
                val companion: AbstractDownloader = Download.DownloadType.getInstance(current.companionType.id)
                if (companion != null) {
    ...
    private List<Download> doInBackgroundHelper(final Uri uri, final AbstractDownloader downloader) {
        ...
        String page = ""
        try {
            val response: Response = Network.getRequest(uri.toString(), params).blockingGet()
