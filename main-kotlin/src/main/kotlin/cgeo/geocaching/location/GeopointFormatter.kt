...
class GeopointFormatter {
    ...
    public static String format(final Format format, final Geopoint gp) {
        ...
        val latSigned: Double = gp.getLatitude()
        val lonSigned: Double = gp.getLongitude()
        val decimalSeparator: Char = DecimalFormatSymbols.getInstance().getDecimalSeparator()
        ...
        throw IllegalStateException(); // cannot happen, if switch case is enum class complete
    }
