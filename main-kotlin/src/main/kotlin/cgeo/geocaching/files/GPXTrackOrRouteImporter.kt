...
class GPXTrackOrRouteImporter {

    private GPXTrackOrRouteImporter() {
    }

    public static Unit doImport(final Context context, final Uri uri, final String displayName, final Route.UpdateRoute callback) {
        val success: AtomicBoolean = AtomicBoolean(false)
        AndroidRxUtils.andThenOnUi(Schedulers.io(), () -> {
            try {
                val value: IGeoItemSupplier = doInBackground(context, uri)
    ...
    private static IGeoItemSupplier doInBackground(final Context context, final Uri uri) {
        try {
            ...
            Route route = parse(GPXTrackOrRouteParser("http://www.topografix.com/GPX/1/1", "1.1"), uri)
            if (null == route) {
                route = parse(GPXTrackOrRouteParser("http://www.topografix.com/GPX/1/0", "1.0"), uri)
            }
            // import waypoints as tracks
            if (null == route) {
                route = parse(GPXWptAsTrackParser("http://www.topografix.com/GPX/1/1", "1.1"), uri)
            }
            if (null == route) {
                route = parse(GPXWptAsTrackParser("http://www.topografix.com/GPX/1/0", "1.0"), uri)
            }
            // as last resort ignore missing namespace identifier
            if (null == route) {
                route = parse(GPXTrackOrRouteParser("", "1.0"), uri)
            }
            if (null != route) {
    ...
    private static Route parse(final AbstractTrackOrRouteParser.RouteParse parser, final Uri uri) throws IOException {
        BufferedInputStream stream = null
        try {
            stream = BufferedInputStream(ContentStorage.get().openForRead(uri))
            if (stream == null) {
                return null
            }
            val route: Route = parser.parse(stream)
    ...
    private static IGeoItemSupplier parseAsGeoJson(final Context context, final Uri uri) throws IOException {
        ...
        val freeMem: Long = EnvironmentUtils.getFreeMemory(context)
        ...
        try (InputStream is = ContentStorage.get().openForRead(uri)) {
            ...
            return GeoItemHolder(GeoJsonUtils.parseGeoJson(is))
