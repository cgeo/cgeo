...
abstract class AbstractImportGpxZipThread : AbstractImportGpxThread() {

    public static val ENCODING: String = "cp437"; // Geocaching.com used windows cp 437 encoding
    private var caches: Collection<Geocache> = Collections.emptySet()
    ...
    override     protected Collection<Geocache> doImport(final GPXParser parser) throws IOException, ParserException {
        ...
        if (!parseZipFileGpxContent(parser, filename -> !StringUtils.endsWithIgnoreCase(filename, GPXImporter.WAYPOINTS_FILE_SUFFIX_AND_EXTENSION),
            ...
            throw ParserException("Imported ZIP does not contain a GPX file.")
        }
    ...
    private Boolean parseZipFileGpxContent(final GPXParser parser, final Predicate<String> filenameMatcher, final Int importStep, final @StringRes Int stepText) throws IOException, ParserException {
        val zisPass: ZipArchiveInputStream = ZipArchiveInputStream(BufferedInputStream(getInputStream()), ENCODING)
        try {
            Int acceptedFiles = 0
            Int ignoredFiles = 0
            for (ZipEntry zipEntry = zisPass.getNextZipEntry(); zipEntry != null; zipEntry = zisPass.getNextZipEntry()) {
                ...
                if (StringUtils.endsWithIgnoreCase(gpxFileName, FileUtils.GPX_FILE_EXTENSION)) {
                    if (filenameMatcher.test(gpxFileName)) {
                        val size: Long = zipEntry.getSize()
                        ...
                        caches = parser.parse(NoCloseInputStream(zisPass), progressHandler)
                        acceptedFiles++
                    }
                } else {
                    ignoredFiles++
