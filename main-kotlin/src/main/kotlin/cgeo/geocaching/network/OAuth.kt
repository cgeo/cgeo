...
class OAuth {
    ...
    public static Unit signOAuth(final String host,
                                 final String path,
                                 final String method,
                                 final Boolean https,
    ...
    static String signCompletedOAuth(final String host, final String path, final String method, final Boolean https,
        ...
        val requestPacked: String = method + '&' + Parameters.percentEncode((https ? "https" : "http") + "://" + host + path) + '&' +
                Parameters.percentEncode(params.toString())
        val keysPacked: String = Parameters.percentEncode(consumerSecret) + '&' + Parameters.percentEncode(StringUtils.defaultString(tokenSecret)); // both even if empty some of them!
