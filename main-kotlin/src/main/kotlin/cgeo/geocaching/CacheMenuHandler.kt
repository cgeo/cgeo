 ...
 */
class CacheMenuHandler : AbstractUIFactory() {
    ...
    public static Boolean onMenuItemSelected(final MenuItem item, final CacheMenuHandler.ActivityInterface activityInterface, final Geocache cache, final Runnable notifyDataSetChanged, final Boolean fromPopup) {
        final Activity activity
        if (activityInterface is Activity) {
        ...
        if (menuItem == R.id.menu_default_navigation) {
            ...
            val navigationProvider: NavigationSelectionActionProvider = (NavigationSelectionActionProvider) MenuItemCompat.getActionProvider(item)
            ...
            if (cache != null && activity != null) {
                val shareText: StringBuilder = StringBuilder().append(cache.getShareSubject()).append("\n").append(cache.getUrl()).append("\n\n")
    ...
    private static Unit setFoundState(final Activity activity, final Geocache cache, final Boolean foundState, final Boolean dnfState, final Runnable notifyDataSetChanged) {
    ...
    public static Unit onPrepareOptionsMenu(final Menu menu, final Geocache cache, final Boolean fromPopup) {
        if (cache == null) {
            return
        }
        ...
        menu.findItem(R.id.menu_upload_bookmarklist).setVisible(Settings.isGCConnectorActive() && Settings.isGCPremiumMember() && ConnectorFactory.getConnector(cache) is GCConnector)
    ...
    public static Unit addMenuItems(final MenuInflater inflater, final Menu menu, final Geocache cache, final Boolean fromPopup) {
    ...
    public static Unit initDefaultNavigationMenuItem(final Menu menu, final INavigationSource navigationSource) {
        val defaultNavigationMenuItem: MenuItem = menu.findItem(R.id.menu_default_navigation)
        val defaultNavAction: NavigationActionProvider = (NavigationActionProvider) MenuItemCompat.getActionProvider(defaultNavigationMenuItem)
