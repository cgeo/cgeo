...
class LogActivityHelper {
    ...
    enum class class ResultType { CREATE, EDIT, DELETE, CREATE_TRACKABLE }
    ...
    public Unit createLog(final Geocache cache, final OfflineLogEntry logEntry, final Map<String, Trackable> inventory) {
        logCreateTask.start(ImmutableTriple<>(cache, logEntry, inventory))
    }

    /** edit a log on the geocaching platform */
    public Unit editLog(final Geocache cache, final LogEntry oldEntry, final LogEntry newEntry) {
        ...
        logEditTask.start(ImmutableTriple<>(cache, oldEntry, newEntry))
    }


    /** delete a log on the geocaching platform */
    public Unit deleteLog(final Geocache cache, final LogEntry entry) {
        if (!LogUtils.canDeleteLog(cache, entry)) {
            //should never happen (would be a programming error), thus not translating message
            ActivityMixin.showToast(activity, "Can't delete log")
            return
        }
        val dialog: SimpleDialog = SimpleDialog.ofContext(activity)
        ...
        if (entry.isOwn()) {
            dialog.confirm(() -> logDeleteTask.start(ImmutableTriple<>(cache, entry, null)))
        } else {
            dialog.input(SimpleDialog.InputOptions().setLabel("Reason"), reasonText -> logDeleteTask.start(ImmutableTriple<>(cache, entry, reasonText)))
        }
    }

    /** create a trackable log on the trackable platform */
    public Unit createLogTrackable(final Geocache cache, final TrackableLogEntry logEntry, final TrackableConnector connector) {
        logCreateTrackableTask.start(ImmutableTriple<>(cache, logEntry, connector))
    }
