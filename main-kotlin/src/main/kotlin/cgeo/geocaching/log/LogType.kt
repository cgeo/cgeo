...
enum class class LogType {
    ...
    private static val FIND_BY_ICONNAME: Map<String, LogType> = HashMap<>()
    private static val FIND_BY_TYPE: Map<String, LogType> = HashMap<>()

    LogType(final Int id, final String iconName, final String type, final String ocType,
    ...
    public static LogType getByIconName(final String imageType) {
        ...
        val result: LogType = imageType != null ? FIND_BY_ICONNAME.get(imageType.toLowerCase(Locale.US).trim()) : null
        if (result == null) {
            return UNKNOWN
        }
        return result
    ...
    public static LogType getByType(final String type) {
        val result: LogType = type != null ? FIND_BY_TYPE.get(type.toLowerCase(Locale.US).trim()) : null
        if (result == null) {
            return UNKNOWN
        }
        return result
    ...
    public static Integer[] getFoundLogIds() {
        return Integer[]{FOUND_IT.id, ATTENDED.id, WEBCAM_PHOTO_TAKEN.id}
    }
    ...
    public static Collection<String> foundLogTypes() {
        val foundLogTypes: ArrayList<String> = ArrayList<>()
