...
class RoutingEngine : Thread() {

    public static val BROUTER_ENGINEMODE_ROUTING: Int = 0
    public static val BROUTER_ENGINEMODE_SEED: Int = 1
    public static val BROUTER_ENGINEMODE_GETELEV: Int = 2

    public Double airDistanceCostFactor
    public SearchBoundary boundary
    protected var waypoints: List<OsmNodeNamed> = null
    protected List<MatchedWaypoint> matchedWaypoints
    protected var foundTrack: OsmTrack = OsmTrack()
    protected var errorMessage: String = null
    protected RoutingContext routingContext
    private NodesCache nodesCache
    private val openSet: SortedHeap<OsmPath> = SortedHeap<>()
    private var finished: Boolean = false
    private var linksProcessed: Int = 0
    private Int nodeLimit; // used for target island search
    private static val MAXNODES_ISLAND_CHECK: Int = 500
    private val islandNodePairs: OsmNodePairSet = OsmNodePairSet(MAXNODES_ISLAND_CHECK)
    private var engineMode: Int = 0
    private static val MAX_STEPS_CHECK: Int = 10
    private var foundRawTrack: OsmTrack = null
    protected var outputMessage: String = null
    ...
    private Unit logInfo(final String s) {
        Log.i(s)
    }

    private Unit logThrowable(final Throwable t) {
    ...
    public Unit doRun(Long maxRunningTime) {
        ...
        switch (engineMode) {
            ...
            case BROUTER_ENGINEMODE_SEED: /* do nothing, handled the old way */
                throw IllegalArgumentException("not a valid engine mode")
    ...
    public Unit doRouting(final Long maxRunningTime) {
        try {
            startTime = System.currentTimeMillis()
            val startTime0: Long = startTime
            this.maxRunningTime = maxRunningTime
            val nsections: Int = waypoints.size() - 1
            final OsmTrack[] refTracks = OsmTrack[nsections]; // used ways for alternatives
            final OsmTrack[] lastTracks = OsmTrack[nsections]
            OsmTrack track = null
    ...
    public Unit doGetElev() {
        try {
            startTime = System.currentTimeMillis()

            routingContext.turnInstructionMode = 9
            val wpt1: MatchedWaypoint = MatchedWaypoint()
            wpt1.waypoint = waypoints.get(0)
            wpt1.name = "wpt_info"
            val listOne: List<MatchedWaypoint> = ArrayList<>()
            listOne.add(wpt1)
            ...
            val distCn1: Int = listOne.get(0).crosspoint.calcDistance(listOne.get(0).node1)
            val distCn2: Int = listOne.get(0).crosspoint.calcDistance(listOne.get(0).node2)
            ...
            val n: OsmNodeNamed = OsmNodeNamed(listOne.get(0).crosspoint)
    ...
    private Unit postElevationCheck(OsmTrack track) {
        ...
        Int endIdx = -1
        Int dist = 0
        val ourSize: Int = track.nodes.size()
        for (Int idx = 0; idx < ourSize; idx++) {
            val n: OsmPathElement = track.nodes.get(idx)
            if (n.getSElev() == Short.MIN_VALUE && lastElev != Short.MIN_VALUE && idx < ourSize - 1) {
                ...
                val diffElev: Int = endElev - startElev
                ...
                for (Int i = startIdx - 1; i < endIdx + 1; i++) {
                    val tmp: OsmPathElement = track.nodes.get(i)
                    if (tmp.message != null) {
                        val md: MessageData = tmp.message.copy()
                        val msg: String = md.wayKeyValues
                        if (!msg == (lastMsg)) {
                            val revers: Boolean = msg.contains("reversedirection=yes")
                            ...
                            if (startincline == 0) {
                                ...
                                val diff: Double = endElev - selev
                                tmpincline = diff / (distRest / 100.)
                            }
                        }
                        lastMsg = msg
                    }
                    val tmpdist: Int = tmp.calcDistance(tmpPt)
                    distRest -= tmpdist
                    if (hasInclineTags) {
                        incline = tmpincline
                    }
    ...
    private Unit logException(final Throwable t) {
        errorMessage = t is IllegalArgumentException ? t.getMessage() : t.toString()
        logInfo("Error (linksProcessed=" + linksProcessed + " open paths: " + openSet.getSize() + "): " + errorMessage)
    }


    public Unit cleanOnOOM() {
        terminate()
    }
    ...
    private OsmTrack tryFindTrack(final OsmTrack[] refTracks, final OsmTrack[] lastTracks) {
        val totaltrack: OsmTrack = OsmTrack()
        ...
        if (!hasDirectRouting && lastTracks[waypoints.size() - 2] == null) {
            val debugInfo: StringBuilder = hasInfo() ? StringBuilder() : null
            ...
            if (hasInfo()) {
                val found: Boolean = nearbyTrack != null
                val dirty: Boolean = found && nearbyTrack.isDirty
        ...
        if (matchedWaypoints == null) { // could exist from the previous alternative level
            ...
            for (Int i = 0; i < matchedWaypoints.size() - 1; i++) {
                ...
                if (routingContext.inverseRouting) {
                    val seg: OsmTrack = findTrack("start-island-check", matchedWaypoints.get(i), matchedWaypoints.get(i + 1), null, null, false)
                    if (seg == null && nodeLimit > 0) {
                        throw IllegalArgumentException("start island detected for section " + i)
                    }
                } else {
                    val seg: OsmTrack = findTrack("target-island-check", matchedWaypoints.get(i + 1), matchedWaypoints.get(i), null, null, false)
                    if (seg == null && nodeLimit > 0) {
                        throw IllegalArgumentException("target island detected for section " + i)
                    }
                }
            }
    ...
    private Boolean snappPathConnection(OsmTrack tt, OsmTrack t, MatchedWaypoint startWp) {
        ...
        val ourSize: Int = tt.nodes.size()
        if (ourSize > 0) {
            if (routingContext.poipoints != null) {
                for (OsmNodeNamed node : routingContext.poipoints) {
                    val dist: Int = node.calcDistance(startWp.crosspoint)
                    if (dist < routingContext.waypointCatchingRange) {
                        return false
                    }
                }
            }
            val removeBackList: List<OsmPathElement> = ArrayList<>()
            val removeForeList: List<OsmPathElement> = ArrayList<>()
            val removeVoiceHintList: List<Integer> = ArrayList<>()
            OsmPathElement last = null
            val lastJunctions: CompactLongMap<OsmTrack.OsmPathElementHolder> = CompactLongMap<>()
            ...
            Int indexfore = 0
            val stop: Int = (Math.max(indexback - MAX_STEPS_CHECK, 1))
            ...
            while (indexback >= 1 && indexback >= stop && indexfore < t.nodes.size()) {
                ...
                val dist: Int = tmpback.calcDistance(tmpfore)
    ...
    private Unit setNewVoiceHint(/* OsmTrack t,*/ OsmPathElement last, /* CompactLongMap<OsmTrack.OsmPathElementHolder> lastJunctiona,*/ OsmPathElement newJunction, OsmPathElement newTarget) {

        if (last == null || newJunction == null || newTarget == null) {
            return
        }
        val lon0: Int = last.getILon()
        val lat0: Int = last.getILat()
        val lon1: Int = newJunction.getILon()
        val lat1: Int = newJunction.getILat()
        val lon2: Int = newTarget.getILon()
        val lat2: Int = newTarget.getILat()
        // get a angle
        val angle: Double = routingContext.anglemeter.calcAngle(lon0, lat0, lon1, lat1, lon2, lat2)

        newTarget.message.turnangle = (Float) angle
    }

    @SuppressWarnings("PMD.NPathComplexity") // external code, do not split
    private Unit recalcTrack(OsmTrack t) {
        Int totaldist = 0
        Float lasttime = 0
        Float speedMin = 9999
        val directMap: Map<Integer, Integer> = HashMap<>()
        Float tmptime = 1
        Float speed = 1
        Int dist
        Double angle
        ...
        Double ascend = 0
        Double ehb = 0.
        val ourSize: Int = t.nodes.size()

        Short eleStart = Short.MIN_VALUE
        Short eleEnd = Short.MIN_VALUE
        val eleFactor: Double = routingContext.inverseRouting ? 0.25 : -0.25

        for (Int i = 0; i < ourSize; i++) {
            val n: OsmPathElement = t.nodes.get(i)
            if (n.message == null) {
            ...
            if (i == 0) {
                ...
                val lon0: Int = t.nodes.get(i - 2).getILon()
                val lat0: Int = t.nodes.get(i - 2).getILat()
                val lon1: Int = t.nodes.get(i - 1).getILon()
                val lat1: Int = t.nodes.get(i - 1).getILat()
                val lon2: Int = t.nodes.get(i).getILon()
                val lat2: Int = t.nodes.get(i).getILat()
...
//            lastenergy = n.getEnergy();
            ...
            if (nLast != null) {
                val eleLast: Short = nLast.getSElev()
                if (eleLast != Short.MIN_VALUE) {
                    ehb = ehb + (eleLast - ele) * eleFactor
                }
                val filter: Double = elevationFilter(n)
                if (ehb > 0) {
                    ascend += ehb
                    ehb = 0
                } else if (ehb < filter) {
        ...
        val keys: SortedSet<Integer> = TreeSet<>(directMap.keySet())
        for (Integer key : keys) {
            val value: Int = directMap.get(key)
            val addTime: Float = (value / (speedMin / 3.6f))

            Double addEnergy = 0
            if (key > 0) {
                val gravity: Double = 9.81;  // in meters per second^(-2)
                val incline: Double = (t.nodes.get(key - 1).getSElev() == Short.MIN_VALUE || t.nodes.get(key).getSElev() == Short.MIN_VALUE ? 0 : (t.nodes.get(key - 1).getElev() - t.nodes.get(key).getElev()) / value)
                val fRoll: Double = routingContext.totalMass * gravity * (routingContext.defaultCR + incline)
                val spd: Double = speedMin / 3.6
                addEnergy = value * (routingContext.sCX * spd * spd + fRoll)
            }
            for (Int j = key; j < ourSize; j++) {
                val n: OsmPathElement = t.nodes.get(j)
    ...
    Double elevationFilter(OsmPos n) {
        if (nodesCache != null) {
            val r: Int = nodesCache.getElevationType(n.getILon(), n.getILat())
            if (r == 1) {
                return -5.
            }
        }
    ...
    private Unit matchWaypointsToNodes(final List<MatchedWaypoint> unmatchedWaypoints) {
        resetCache(false)
    ...
    private OsmTrack searchTrack(final MatchedWaypoint startWp, final MatchedWaypoint endWp, final OsmTrack nearbyTrack, final OsmTrack refTrack) {
        ...
        try {
            val calcBeeline: Boolean = startWp.direct
            ...
            OsmPath path = routingContext.createPath(OsmLink(null, startWp.crosspoint))
    ...
    private OsmTrack searchRoutedTrack(MatchedWaypoint startWp, MatchedWaypoint endWp, OsmTrack nearbyTrack, OsmTrack refTrack) {
        OsmTrack track = null
        final Double[] airDistanceCostFactors = Double[]{
        ...
        if (track == null) {
            throw IllegalArgumentException("no track found")
        }

        val wasClean: Boolean = nearbyTrack != null && !nearbyTrack.isDirty
        if (refTrack == null && !(wasClean && isDirty)) { // do not overwrite a clean with a dirty track
            logInfo("supplying reference track, dirty=" + isDirty)
        ...
        try {
            val tt: OsmTrack = findTrack("re-tracking", startWp, endWp, null, refTrack, false)
            if (tt == null) {
                throw IllegalArgumentException("error re-tracking track")
            }
            return tt
        } finally {
            guideTrack = null
    ...
    private Unit resetCache(final Boolean detailed) {
        if (hasInfo() && nodesCache != null) {
            logInfo("NodesCache status before reset=" + nodesCache.formatStatus())
        }
        val maxmem: Long = routingContext.memoryclass * 1024L * 1024L; // in MB

        nodesCache = NodesCache(routingContext.expctxWay, maxmem, nodesCache, detailed)
    ...
    private OsmPath getStartPath(final OsmNode n1, final OsmNode n2, final MatchedWaypoint mwp, final OsmNodeNamed endPos, final Boolean sameSegmentSearch) {
        ...
        val p: OsmPath = getStartPath(n1, n2, OsmNodeNamed(mwp.crosspoint), endPos, sameSegmentSearch)
    ...
    private OsmPath getStartPath(final OsmNode n1, final OsmNode n2, final OsmNodeNamed wp, final OsmNodeNamed endPos, final Boolean sameSegmentSearch) {
        try {
            ...
            val startLink: OsmLink = OsmLink(null, n1)
            val startPath: OsmPath = routingContext.createPath(startLink)
            startLink.addLinkHolder(startPath, null)
            Double minradius = 1e10
            for (OsmLink link = n1.firstlink; link != null; link = link.getNext(n1)) {
                val nextNode: OsmNode = link.getTarget(n1)
                ...
                wp.radius = 1.5
                val testPath: OsmPath = routingContext.createPath(startPath, link, null, guideTrack != null)
    ...
    private OsmTrack findTrack(final String operationName, final MatchedWaypoint startWp, final MatchedWaypoint endWp, final OsmTrack costCuttingTrack, final OsmTrack refTrack, final Boolean fastPartialRecalc) {
        try {
            val wpts2: List<OsmNode> = ArrayList<>()
            if (startWp != null) {
            ...
            val detailed: Boolean = guideTrack != null
            resetCache(detailed)
    ...
    private OsmTrack findTrackHelper(final String operationName, final MatchedWaypoint startWp, final MatchedWaypoint endWp, final OsmTrack costCuttingTrack, final OsmTrack refTrack, Boolean fastPartialRecalc) {
        ...
        val startNodeId1: Long = startWp.node1.getIdFromPos()
        val startNodeId2: Long = startWp.node2.getIdFromPos()
        val endNodeId1: Long = endWp == null ? -1L : endWp.node1.getIdFromPos()
        val endNodeId2: Long = endWp == null ? -1L : endWp.node2.getIdFromPos()
        OsmNode end1 = null
        OsmNode end2 = null
        OsmNodeNamed endPos = null

        Boolean sameSegmentSearch = false
        val start1: OsmNode = nodesCache.getGraphNode(startWp.node1)
        val start2: OsmNode = nodesCache.getGraphNode(startWp.node2)
        if (endWp != null) {
        ...
        val startPath1: OsmPath = getStartPath(start1, start2, startWp, endPos, sameSegmentSearch)
        val startPath2: OsmPath = getStartPath(start2, start1, startWp, endPos, sameSegmentSearch)

        // check for an INITIAL match with the cost-cutting-track
        if (costCuttingTrack != null) {
            val pe1: OsmPathElement = costCuttingTrack.getLink(startNodeId1, startNodeId2)
            if (pe1 != null) {
            ...
            val pe2: OsmPathElement = costCuttingTrack.getLink(startNodeId2, startNodeId1)
            if (pe2 != null) {
        ...
        val openBorderList: List<OsmPath> = ArrayList<>(4096)
        Boolean memoryPanicMode = false
        Boolean needNonPanicProcessing = false

        for (; ; ) {
            if (terminated) {
                throw IllegalArgumentException("operation killed by thread-priority-watchdog after " + (System.currentTimeMillis() - startTime) / 1000 + " seconds")
            }

            if (maxRunningTime > 0) {
                val timeout: Long = (matchPath == null && fastPartialRecalc) ? maxRunningTime / 3 : maxRunningTime
                if (System.currentTimeMillis() - startTime > timeout) {
                    throw IllegalArgumentException(operationName + " timeout after " + (timeout / 1000) + " seconds")
                }
            }
            ...
            synchronized (openSet) {
                ...
                if (directWeaving && nodesCache.hasHollowLinkTargets(path.getTargetNode())) {
                    if (!memoryPanicMode && !nodesCache.nodesMap.isInMemoryBounds(openSet.getSize(), false)) {
                        val nodesBefore: Int = nodesCache.nodesMap.nodesCreated
                        val pathsBefore: Int = openSet.getSize()
                        ...
                        if (!nodesCache.nodesMap.isInMemoryBounds(openSet.getSize(), true)) {
                            if (maxTotalCost < 1000000000 || needNonPanicProcessing || fastPartialRecalc) {
                                throw IllegalArgumentException("memory limit reached")
                            }
                ...
                val currentLink: OsmLink = path.getLink()
                val sourceNode: OsmNode = path.getSourceNode()
                val currentNode: OsmNode = path.getTargetNode()

                if (currentLink.isLinkUnused()) {
                    continue
                }

                val currentNodeId: Long = currentNode.getIdFromPos()
                val sourceNodeId: Long = sourceNode.getIdFromPos()
                ...
                if (path.treedepth != 1) {
                    ...
                    if ((sourceNodeId == endNodeId1 && currentNodeId == endNodeId2)
                        ...
                        val t: OsmTrack = compileTrack(path)
                    ...
                    if (costCuttingTrack != null) {
                        val pe: OsmPathElement = costCuttingTrack.getLink(sourceNodeId, currentNodeId)
                        if (pe != null) {
                            ...
                            val c: Int = path.cost - parentcost - pe.cost
                            if (c > 0) {
                                parentcost += c
                            }
                            ...
                            val costEstimate: Int = path.cost
                ...
                val firstLinkHolder: OsmLinkHolder = currentLink.getFirstLinkHolder(sourceNode)
                ...
                if (path.treedepth > 1) {
                    val isBidir: Boolean = currentLink.isBidirectional()
                ...
                for (OsmLink link = currentNode.firstlink; link != null; link = link.getNext(currentNode)) {
                    val nextNode: OsmNode = link.getTarget(currentNode)
                    ...
                    val prePath: OsmPrePath = routingContext.createPrePath(path, link)
                    if (prePath != null) {
                ...
                for (OsmLink link = currentNode.firstlink; link != null; link = link.getNext(currentNode)) {
                    ...
                    if (guideTrack != null) {
                        val gidx: Int = path.treedepth + 1
                        if (gidx >= guideTrack.nodes.size()) {
                            continue
                        }
                        val guideNode: OsmPathElement = guideTrack.nodes.get(routingContext.inverseRouting ? guideTrack.nodes.size() - 1 - gidx : gidx)
                        val nextId: Long = nextNode.getIdFromPos()
                        if (nextId != guideNode.getIdFromPos()) {
                            ...
                            if (routingContext.turnInstructionMode > 0) {
                                val detour: OsmPath = routingContext.createPath(path, link, refTrack, true)
                    ...
                    for (OsmLinkHolder linkHolder = firstLinkHolder; linkHolder != null; linkHolder = linkHolder.getNextForLink()) {
                        ...
                        try {
                            ...
                            val testPath: OsmPath = routingContext.createPath(otherPath, link, refTrack, guideTrack != null)
                    ...
                    if (bestPath != null) {
                        ...
                        val inRadius: Boolean = boundary == null || boundary.isInBoundary(nextNode, bestPath.cost)
                        ...
                        if (inRadius && (isFinalLink || bestPath.cost + bestPath.airdistance <= (lastAirDistanceCostFactor != 0. ? maxTotalCost * lastAirDistanceCostFactor : maxTotalCost) + addDiff)) {
                            ...
                            while (dominator != null) {
                                val dp: OsmPath = (OsmPath) dominator
    ...
    private Unit addToOpenset(final OsmPath path) {
        if (path.cost >= 0) {
    ...
    private OsmTrack compileTrack(final OsmPath path) {
        ...
        val totalTime: Float = element.getTime()
        val totalEnergy: Float = element.getEnergy()
        ...
        while (element != null) {
            ...
            val nextElement: OsmPathElement = element.origin
    ...
    private OsmTrack mergeTrack(final OsmPathElement match, final OsmTrack oldTrack) {
        ...
        Long lastId = 0
        val id1: Long = match.getIdFromPos()
        val id0: Long = match.origin == null ? 0 : match.origin.getIdFromPos()
