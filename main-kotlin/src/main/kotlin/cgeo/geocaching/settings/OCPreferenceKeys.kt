...
enum class class OCPreferenceKeys {
    ...
    static {
        FIND_BY_ISACTIVE_ID = SparseArray<>(values().length)
        FIND_BY_AUTH_PREF_ID = SparseArray<>(values().length)
        val byIsactiveKey: Map<String, OCPreferenceKeys> = HashMap<>()
