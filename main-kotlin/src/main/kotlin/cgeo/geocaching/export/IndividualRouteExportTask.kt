...
class IndividualRouteExportTask : AsyncTaskWithProgress()<RouteSegment, Uri> {
    private final String filename
    private final Boolean exportAsTrack

    private static val PREFIX_GPX: String = ""
    private static val NS_GPX: String = "http://www.topografix.com/GPX/1/1"
    ...
    override     protected Uri doInBackgroundInternal(final RouteSegment[] trail) {
        val uri: Uri = ContentStorage.get().create(PersistableFolder.GPX, filename)
        if (uri == null) {
            return null
        }
        val gpx: XmlSerializer = KXmlSerializer()
        Writer writer = null
        try {
            ...
            val timeInfo: String = CalendarUtils.formatDateTime("yyyy-MM-dd") + "T" + CalendarUtils.formatDateTime("hh:mm:ss") + "Z"
            ...
            for (RouteSegment loc : trail) {
                val segmentName: String = loc.getItem().getIdentifier()
                if (exportAsTrack) {
                    val points: ArrayList<Geopoint> = loc.getPoints()
    ...
    private Unit exportPoint(final XmlSerializer gpx, final String tag, final Geopoint point, final String name) throws IOException {
    ...
    override     protected Unit onPostExecuteInternal(final Uri exportUri) {
        if (null != activity) {
            if (null != exportUri) {
