...
class TrailHistoryExportTask : AsyncTaskWithProgress()<TrailHistoryElement, Uri> {
    ...
    override     protected Uri doInBackgroundInternal(final TrailHistoryElement[] trail) {
        ...
        try {
            ...
            try {
                ...
                val formatter: SimpleDateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'", Locale.US)
                formatter.setTimeZone(TimeZone.getTimeZone("GMT"))

                gpx.startTag(NS_GPX, "metadata")
                XmlUtils.simpleText(gpx, NS_GPX, "name", "c:geo history trail")
                XmlUtils.simpleText(gpx, NS_GPX, "time", formatter.format(Date()))
                gpx.endTag(NS_GPX, "metadata")

                gpx.startTag(NS_GPX, "trk")
                XmlUtils.simpleText(gpx, NS_GPX, "name", "c:geo history trail " + formatter.format(Date()))
