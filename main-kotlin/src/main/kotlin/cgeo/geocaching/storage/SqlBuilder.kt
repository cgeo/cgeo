...
class SqlBuilder {
    ...
    enum class class WhereType { AND, OR, NOT }

    private var tableIdCnt: Int = 1

    private final String mainTable
    private final String[] columns


    private val tables: List<String> = ArrayList<>()
    private val joins: List<String> = ArrayList<>()
    private val orders: List<String> = ArrayList<>()

    private final Stack<ImmutableTriple<WhereType, StringBuilder, List<String>>> whereStack = Stack<>()
    private var whereInvertTrue: Boolean = false
    ...
    public SqlBuilder(final String mainTable, final String[] columns) {
        ...
        whereStack.push(ImmutableTriple<>(WhereType.AND, StringBuilder(), ArrayList<>()))
    }
    ...
    public SqlBuilder openWhere(final WhereType operator) {
        whereStack.push(ImmutableTriple<>(operator, StringBuilder(), ArrayList<>()))
    ...
    public String[] getSqlWhereArgsArray() {
        return getSqlWhereArgs().toArray(String[0])
    }
    ...
    private String constructSqlInternal(final String columnString, final Boolean includeOrderBy, final Boolean includeLimit) {
        ...
        val sb: StringBuilder = StringBuilder("SELECT " + columnString + " FROM " + mainTable + " " + getMainTableId())
