...
class OneTimeDialogs : DataStore().DBExtension {
    ...
    /**
     * list of all one time dialogs - dialogs must be defined here
     */
    enum class class DialogType {
     ...
     */
    enum class class DefaultBehavior {
        SHOW_ALWAYS,
    ...
    public static Boolean showDialog(final DialogType dialogType) {
        ...
        if (null != temp) {
            val dialog: OneTimeDialogs = OneTimeDialogs(temp)
            val status: DialogStatus = getStatusById((Int) dialog.getLong1())
     ...
     */
    public static Unit setStatus(final DialogType dialogType, final DialogStatus status) {
     ...
     */
    public static Unit setStatus(final DialogType dialogType, final DialogStatus currentStatus, final DialogStatus nextStatus) {
    ...
    public static Unit initializeOnFreshInstall() {
        for (DialogType key : DialogType.values()) {
