...
class ContentStorage {
    ...
    private val reportRunningFlag: ThreadLocal<Boolean> = ThreadLocal<>()

    private static val INSTANCE: ContentStorage = ContentStorage()
    ...
    private ContentStorage() {
        try (ContextLogger ignored = ContextLogger(true, "ContentStorage.init")) {
            this.context = CgeoApplication.getInstance().getApplicationContext()
            this.documentAccessor = DocumentContentAccessor(this.context)
            this.fileAccessor = FileContentAccessor(this.context)
    ...
    public Boolean ensureFolder(final PersistableFolder publicFolder) {
        val folder: Folder = getFolder(publicFolder, false)
        if (folder == null) {
            return false
        }
    ...
    /**
     * Creates a file in folder and returns its Uri
     */
    ...
    /**
     * Creates a file in a folder location and returns its Uri
     */
    public Uri create(final Folder folder, final FileNameCreator nameCreator, final Boolean onlyIfNotExisting) {
        ...
        val name: String = (nameCreator == null ? FileNameCreator.DEFAULT : nameCreator).createName()
        if (onlyIfNotExisting) {
            val fi: FileInformation = getFileInfo(folder, name)
            if (fi != null) {
                return fi.uri
            }
        }
    ...
    /**
     * Renames the file represented by given Uri. This might change the Uri, the Uri is returned.
    ...
    public List<FileInformation> list(final Folder folder, final Boolean sortByName, final Boolean suppressWarningForUser) {
        try (ContextLogger cLog = ContextLogger("ContentStorage.list: %s", folder)) {
            if (folder == null) {
                return Collections.emptyList()
            }
            val result: List<FileInformation> = getAccessorFor(folder).list(folder)
    ...
    public FileInformation getFileInfo(final Folder rootFolder, final String subfolderAndName) {
        val info: ImmutablePair<FileInformation, Folder> = getParentFolderAndFileInfo(rootFolder, subfolderAndName)
    ...
    public ImmutablePair<FileInformation, Folder> getParentFolderAndFileInfo(final Folder rootFolder, final String subfolderAndName) {
        ...
        val idx: Int = subfolderAndName.lastIndexOf("/")
        if (idx < 0) {
        ...
        try {
            val fileInfo: FileInformation = getAccessorFor(parentFolder).getFileInfo(parentFolder, fileName)
            return ImmutablePair<>(fileInfo, parentFolder)
        } catch (IOException ioe) {
    ...
    public String getName(final Uri uri) {
        val fi: FileInformation = getFileInfo(uri)
    ...
    public InputStream openForRead(final Folder folder, final String subfolderAndName) {
        ...
        val fi: FileInformation = getFileInfo(folder, subfolderAndName)
        if (fi != null) {
    ...
    public File createTempFile(final String fileName) {
        ...
        try {
            ...
            val tempFile: File = File(outputDir, fileName)
    ...
    /**
     * Sets a User-defined Folder for a {@link PersistableFolder}.
     */
    public Unit setUserDefinedFolder(final PersistableFolder folder, final Folder userDefinedFolder, final Boolean setByUser) {
        folder.setUserDefinedFolder(userDefinedFolder, setByUser)
        documentAccessor.releaseOutdatedUriPermissions()
        ensureFolder(folder)
        //in case this shifts other default folders (e.g. when MAPS-folder is changed then default THEME folder might need creation)
        PersistableFolder.reevaluateDefaultFolders()
    }

    public Unit setPersistedDocumentUri(final PersistableUri persistedDocUi, final Uri uri) {
    ...
    private Boolean tryTestReadWriteToFolder(final Folder folder, final Boolean testWrite) {
        ...
        try {
            ...
            val files: List<FileInformation> = list(folder)
    ...
    private Unit reportProblem(@StringRes final Int messageId, final Boolean suppressForUser, final Object... params) {
        reportProblem(messageId, null, suppressForUser, params)
    }

    private Unit reportProblem(@StringRes final Int messageId, final Exception ex, final Boolean suppressForUser, final Object... params) {

        if (reportRunningFlag.get() != null && Boolean.TRUE == (reportRunningFlag.get())) {
            return
        }
        reportRunningFlag.set(true)

        //prepare params message
        val messages: ImmutablePair<String, String> = LocalizationUtils.getMultiPurposeString(messageId, "ContentStorage", params)
