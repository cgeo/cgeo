...
class PositionHistoryLayer {

    private final GeoItemLayer<String> layer
    private var pathStored: Boolean = true
    ...
    private static val KEY_HISTORY_LINE: String = "historyLine"
    ...
    private Unit drawHistory(final Location currentLoc) {
        ...
        val history: PositionHistory = viewModel.positionHistory.getValue()
        ...
        val historyElements: ArrayList<TrailHistoryElement> = history.getHistory()
        final GeoGroup.Builder geoGroup = GeoGroup.builder()

        ArrayList<Geopoint> segmentPoints = ArrayList<>()

        segmentPoints.add(Geopoint(historyElements.get(0).getLocation()))
        for (Int i = 1; i < historyElements.size(); i++) {
            if (historyElements.get(i).getLocation().distanceTo(historyElements.get(i - 1).getLocation()) > LINE_MAXIMUM_DISTANCE_METERS) {
                ...
                segmentPoints = ArrayList<>()
            }
            segmentPoints.add(Geopoint(historyElements.get(i).getLocation()))
        }
        // always add current position
        if (currentLoc != null) {
            segmentPoints.add(Geopoint(currentLoc))
        }
