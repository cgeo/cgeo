...
class PositionLayer {
    ...
    private static val KEY_POSITION: String = "positionMarker"
    private static val KEY_ACCURACY: String = "accuracy"
    private static val KEY_ELEVATION: String = "elevation"

    private val markerPosition: Bitmap = ImageUtils.convertToBitmap(ResourcesCompat.getDrawable(CgeoApplication.getInstance().getResources(), R.drawable.my_location_chevron, null))

    private val accuracyStyle: GeoStyle = GeoStyle.builder()
            .setStrokeWidth(1.0f)
    ...
    public PositionLayer(final AppCompatActivity activity, final GeoItemLayer<String> layer) {
        ...
        viewModel.location.observe(activity, locationWrapper -> {
            ...
            if (locationWrapper.needsRepaintForDistanceOrAccuracy || !layer.isShown(KEY_ACCURACY)) {
                layer.put(KEY_ACCURACY, GeoPrimitive.createCircle(Geopoint(locationWrapper.location),
            ...
            layer.put(KEY_POSITION,
                    GeoPrimitive.createMarker(Geopoint(locationWrapper.location), GeoIcon.builder()
