...
class GeoItemsLayer {
    ...
    private var lastDisplayedGeocaches: Map<String, String> = HashMap<>()
    private var lastDisplayedWaypoints: Map<String, String> = HashMap<>()
    private val lastDisplayedCacheStars: CollectionDiff<String, String, String> = CollectionDiff<>(k -> k)
    private var lastForceCompactIconMode: Boolean = false
    ...
    public GeoItemsLayer(final AppCompatActivity activity, final GeoItemLayer<String> layer) {
        ...
        viewModel.caches.observeForRead(activity, caches -> { // this is always executed on UI thread, thus doesn't need to be thread save
            ...
            val currentlyDisplayedGeocaches: Map<String, String> = HashMap<>()

            val forceCompactIconMode: Boolean = CompactIconModeUtils.forceCompactIconMode()
            ...
            for (Geocache cache : caches) {
                val cm: CacheMarker = forceCompactIconMode ? MapMarkerUtils.getCacheDotMarker(activity.getResources(), cache) : MapMarkerUtils.getCacheMarker(activity.getResources(), cache, null, true)
                val contentKey: String = getKeyFor(cache, cm)
                currentlyDisplayedGeocaches.put(cache.getGeocode(), contentKey)

                if (!lastDisplayedGeocaches.containsKey(cache.getGeocode()) || !lastDisplayedGeocaches.get(cache.getGeocode()) == (contentKey)) {
        ...
        viewModel.cachesWithStarDrawn.observeForRead(activity, starCodes -> lastDisplayedCacheStars.executeDiff(starCodes, true, addStar -> {
            val cache: Geocache = DataStore.loadCache(addStar, LoadFlags.LOAD_CACHE_OR_DB)
            val star: GeoItem = MapStarUtils.createStar(cache)
            if (star != null) {
        ...
        viewModel.waypoints.observeForRead(activity, waypoints -> { // this is always executed on UI thread, thus doesn't need to be thread save
            ...
            val currentlyDisplayedWaypoints: Map<String, String> = HashMap<>()

            for (Waypoint waypoint : waypoints) {
                val cm: CacheMarker = lastForceCompactIconMode ? MapMarkerUtils.getWaypointDotMarker(activity.getResources(), waypoint) : MapMarkerUtils.getWaypointMarker(activity.getResources(), waypoint, true, true)
                val contentKey: String = getKeyFor(waypoint, cm)
                currentlyDisplayedWaypoints.put(waypoint.getFullGpxId(), contentKey)

                if (!lastDisplayedWaypoints.containsKey(waypoint.getFullGpxId()) || !lastDisplayedWaypoints.get(waypoint.getFullGpxId()) == (contentKey)) {
