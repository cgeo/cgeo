...
class MapsforgeThemeHelper : XmlRenderThemeMenuCallback {
    ...
    protected Unit reapplyMapTheme(final Map map, final AbstractTileProvider tileProvider) {
        if (mTheme != null) {
            disposeTheme()
        }
        ...
        if (selectedTheme == null) {
            ...
            try {
                //get the theme
                val xmlRenderTheme: ThemeFile = createThemeFor(selectedTheme)
                ...
                */
                ...
                if (Settings.getMapBackgroundMapLayer() && Settings.isFeatureEnabledDefaultFalse(R.string.pref_vtmBackgroundTransparent)) {
                    map.setTheme(ThemeLoader.load(xmlRenderTheme, ThemeCallbackAdapter() {
                        override                         public Int getColor(final String[] keys, final String[] values, final Int color) {
                            val k: List<String> = Arrays.asList(keys)
                            val v: List<String> = Arrays.asList(values)
        ...
        if (tileProvider is AbstractMapsforgeVTMOfflineTileProvider) {
    ...
    private Unit applyDefaultTheme(final Map map, final AbstractTileProvider tileProvider) {
        if (tileProvider.supportsThemes()) {
            mTheme = map.setTheme(VtmThemes.getDefaultVariant())
        }
    }

    protected Unit disposeTheme() {
        if (mTheme != null) {
            mTheme.dispose()
        }
    }

    private ThemeFile createThemeFor(final ThemeData theme) {
        ...
        try {
            if (!isZipTheme) {
                if (UriUtils.isFileUri(theme.fileInfo.uri)) {
                    xmlRenderTheme = ExternalRenderTheme(theme.fileInfo.uri.toString(), this)
                } else {
    ...
    protected Unit selectMapTheme(final Activity activity, final Map map, final AbstractTileProvider tileProvider) {
        if (!tileProvider.supportsThemes()) {
            return
        }
        val currentThemeId: String = Settings.getSelectedMapRenderTheme(tileProvider)
    ...
    public Unit selectMapThemeOptions(final Activity activity, final AbstractTileProvider tileProvider) {
        ...
        if (themeOptionsAvailable() && themeStyleMenu != null) {
            ...
            intent.putExtra(MapsforgeThemeSettingsFragment.SHOW3DOPTION, tileProvider is AbstractMapsforgeVTMOfflineTileProvider)
        }
