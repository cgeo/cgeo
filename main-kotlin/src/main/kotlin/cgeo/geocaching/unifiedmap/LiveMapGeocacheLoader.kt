...
class LiveMapGeocacheLoader {

    private static val LOGPRAEFIX: String = "LiveMapGeocacheLoader:"

    public static val PROCESS_DELAY: Long = 3000; // value is in milliseconds
    private static val CACHE_EXPIRY: Long = 10 * 60000; // value is in milliseconds

    private final Disposable actionDisposable

    private var dirtyTime: Long = -1
    private Viewport viewport
    private GeocacheFilter filter

    enum class class LoadState { REQUESTED, RUNNING, STOPPED }


    public static class LiveDataState {
    ...
    public static class ConnectorState {
        ...
        public String toUserDisplayableStringWithMarkup() {
            val postfix: String = (message == null ? "" : " (" + message + ")") + (duration <= 0 ? "" : " (~" + Math.round(duration / 1000d) + "s)")
            val prefix: String = "**" + connectorName + "**: "
            if (isError()) {
    ...
    private static class Action : Runnable {
        ...
        private var cachedResultWasComplete: Boolean = true
        private Geopoint cachedResultCenter

        private var lastConnectorStates: Map<String, ConnectorState> = Collections.emptyMap()
        ...
        override         public Unit run() {
            ...
            try {
                ...
                synchronized (loader) {
                    ...
                    val cacheIsValidForFilter: Boolean = GeocacheFilter.filtersSame(cachedResultFilter, filter) && (System.currentTimeMillis() - cachedResultTs) < CACHE_EXPIRY
                    ...
                    if (cacheIsValidForFilter && !cachedResultWasComplete && cachedResultCenter != null) {
                        val distanceCachedCall: Float = viewport.getCenter().distanceTo(cachedResultCenter)
                        val distanceCenterCorner: Float = viewport.getCenter().distanceTo(viewport.bottomLeft)
                    ...
                    val requestAge: Long = System.currentTimeMillis() - this.loader.dirtyTime
                ...
                val stateData: Map<String, ConnectorState> = Collections.synchronizedMap(HashMap<>())
                // DO ONLINE REQUEST retrieving live caches for x times the requested size (to fill up cache)
                val retrievalViewport: Viewport = viewport.resize(3.0)
                val startTs: Long = System.currentTimeMillis()
                final Boolean[] isOverallComplete = Boolean[] { true }
                ConnectorFactory.searchByViewport(retrievalViewport, filter, (c, sr) -> {
                    ...
                    val countOriginal: Int = sr.getCount()
                    val result: Set<Geocache> = sr.getCachesFromSearchResult(LoadFlags.LOAD_CACHE_OR_DB)
                    val countInDb: Int = result.size()
                    if (filter != null) {
                        filter.filterList(result)
                    }
                    val countFiltered: Int = result.size()
                    onResult.accept(result)

                    //collect caching information
                    val isPartial: Boolean = !sr.getPartialConnectors().isEmpty()
                    val errorCode: StatusCode = sr.getError()
                    ...
                    //set state data
                    stateData.put(c.getName(), ConnectorState(c.getName(), errorCode, connectorViewport, countOriginal, countInDb, countFiltered, duration, null))
                })
                ...
                setState(LoadState.STOPPED, Collections.singletonMap("Overall",
                    ConnectorState("Overall", StatusCode.UNKNOWN_ERROR, null, 0, 0, 0, -1, "Exception: " + e.getMessage())))
            }
        ...
        private Unit setState(final LoadState newState) {
            setState(newState, null)
        }

        private Unit setState(final LoadState newState, final Map<String, ConnectorState> connStates) {
            if (connStates != null) {
                this.lastConnectorStates = connStates
            }
            val ldState: LiveDataState = LiveDataState(newState, this.cachedResultAvailableViewport, this.lastConnectorStates)
    ...
    public LiveMapGeocacheLoader(final Consumer<LiveDataState> onStateChanged, final Consumer<Set<Geocache>> onResult) {
        this.actionDisposable = Schedulers.newThread().schedulePeriodicallyDirect(Action(this, onStateChanged, onResult), 0, 250, TimeUnit.MILLISECONDS)
    }

    public synchronized Unit requestUpdate(final Viewport viewport, final GeocacheFilter filter, final Boolean skipDelay) {
        this.viewport = viewport
        this.filter = filter
        val timeMod: Long = skipDelay ? PROCESS_DELAY : 0
        this.dirtyTime = this.dirtyTime <= -1 ? System.currentTimeMillis() - timeMod : this.dirtyTime - timeMod
    }

    public synchronized Unit cancelRequest() {
        this.dirtyTime = -1
    }
