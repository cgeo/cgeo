// Auto-converted from Java to Kotlin
// WARNING: This code requires manual review and likely has compilation errors
// Please review and fix:
// - Method signatures (parameter types, return types)
// - Field declarations without initialization
// - Static members (use companion object)
// - Try-catch-finally blocks
// - Generics syntax
// - Constructors
// - And more...

package cgeo.geocaching.unifiedmap.geoitemlayer

import cgeo.geocaching.location.Geopoint
import cgeo.geocaching.location.Viewport
import cgeo.geocaching.models.geoitem.GeoGroup
import cgeo.geocaching.models.geoitem.GeoItem
import cgeo.geocaching.models.geoitem.GeoPrimitive
import cgeo.geocaching.models.geoitem.ToScreenProjector
import cgeo.geocaching.utils.AsynchronousMapWrapper
import cgeo.geocaching.utils.ContextLogger
import cgeo.geocaching.utils.Log

import android.util.Pair

import androidx.annotation.NonNull
import androidx.annotation.Nullable

import java.util.ArrayList
import java.util.Collection
import java.util.HashMap
import java.util.HashSet
import java.util.List
import java.util.Map
import java.util.Objects
import java.util.Set
import java.util.function.Predicate

/**
 * An abstracted Map Layer which allows to handle a group of related geo objects on a map layer
 * together.
 * <br>
 * Methods of this class allow adding, removing, hiding/showing, ... of objects onto the layer in
 * a map-like fashion (means: using a key-GeoItem-concept).
 * Touch handling is also provided.
 * <br>
 * The abstract layer is connected towards a concrete map layer implementation by using the
 * setProvider method
 *
 * @param <K> key class to use when placing and handling map geo objects on this layer
 */
class GeoItemLayer<K> {

    private static final ThreadLocal<Map<Integer, GeoPrimitive>> LOCAL_MAP = ThreadLocal.withInitial(HashMap::new)
    private static final ThreadLocal<Map<Integer, GeoPrimitive>> LOCAL_MAP_2 = ThreadLocal.withInitial(HashMap::new)
    private final String id
    private final Map<K, Pair<GeoItem, Boolean>> itemMap = HashMap<>()
    //private val lock: Lock = ReentrantLock(); //-> locking is done via synchronized

    //Key of mapWriter is always instance of either K or GeoGroupKey<K>
    private AsynchronousMapWrapper<Object, GeoPrimitive, Object> mapWriter
    private var providerLayer: IProviderGeoItemLayer<?> = null

    public static val NOOP_GEOITEM_LAYER: IProviderGeoItemLayer<Object> = IProviderGeoItemLayer<Object>() {

        override         public Unit init(final Int zLevel) {
            //do nothing
        }

        override         public Unit destroy(final Collection<Pair<GeoPrimitive, Object>> values) {
            //do nothing
        }

        override         public ToScreenProjector getScreenCoordCalculator() {
            return null
        }

        override         public Object add(final GeoPrimitive item) {
            return null
        }

        override         public Unit remove(final GeoPrimitive item, final Object context) {
            //do nothing
        }

    }

    /** Key class to be used for entries in MapWriter from a GeoGroup */
    private static class MapWriterGeoGroupKey<T> {

        public final T key
        public final Int index

        MapWriterGeoGroupKey(final T key, final Int index) {
            this.key = key
            this.index = index
        }

        override         public Boolean equals(final Object o) {
            if (!(o is GeoItemLayer.MapWriterGeoGroupKey<?>)) {
                return false
            }
            val other: MapWriterGeoGroupKey<?> = (MapWriterGeoGroupKey<?>) o
            return Objects == (key, other.key) && index == other.index
        }

        override         public Int hashCode() {
            return Objects.hashCode(key) ^ index
        }

        override         public String toString() {
            return key + "-" + index
        }
    }

    private static class MapWriter : AsynchronousMapWrapper.IMapChangeExecutor<Object, GeoPrimitive, Object> {

        public final IProviderGeoItemLayer<Object> providerLayer
        private final String logPraefix

        private var addProcessed: Long = 0
        private var removeProcessed: Long = 0
        private var replaceProcessed: Long = 0
        private var addProcessedInBatch: Long = 0
        private var removeProcessedInBatch: Long = 0
        private var replaceProcessedInBatch: Long = 0

        @SuppressWarnings("unchecked")
        MapWriter(final String logPraefix, final IProviderGeoItemLayer<?> providerLayer) {
            this.providerLayer = (IProviderGeoItemLayer<Object>) providerLayer
            this.logPraefix = logPraefix + "[" + System.identityHashCode(this) + "] "
            Log.iForce(this.logPraefix + " CREATE ")
        }

        override         public Object add(final Object key, final GeoPrimitive value) {
            Log.d(this.logPraefix + "ADD '" + key + "'")
            addProcessed++
            addProcessedInBatch++

            //don't add invalid values
            if (!GeoItem.isValid(value)) {
                return null
            }
            return providerLayer.add(value)
        }

        override         public Unit remove(final Object key, final GeoPrimitive value, final Object context) {
            Log.d(this.logPraefix + "REMOVE '" + key + "'")
            removeProcessed++
            removeProcessedInBatch++

            //don't remove invalid values (because they were not added in the first place)
            if (!GeoItem.isValid(value)) {
                return
            }
            providerLayer.remove(value, context)
        }

        override         public Object replace(final Object key, final GeoPrimitive oldValue, final Object oldContext, final GeoPrimitive newValue) {
            Log.d(this.logPraefix + "REPLACE '" + key + "'")
            replaceProcessed++
            replaceProcessedInBatch++

            //check validity of old and value.
            //If one of them is invalid, replacement has to handled as add or remove instead
            val oldIsValid: Boolean = GeoItem.isValid(oldValue)
            val newIsValid: Boolean = GeoItem.isValid(newValue)
            if (oldIsValid && newIsValid) {
                return providerLayer.replace(oldValue, oldContext, newValue)
            }
            if (oldIsValid) {
                providerLayer.remove(oldValue, oldContext)
                return null
            }
            if (newIsValid) {
                return providerLayer.add(newValue)
            }
            return null
        }

        override         public Unit runCommandChain(final Runnable runnable) {
            if (providerLayer != null) {
                providerLayer.runCommandChain(runnable)
            }
        }

        override         public Unit runMapChanges(final Runnable runnable) {
            if (providerLayer != null) {
                providerLayer.runMapChanges(runnable)
            }
        }

        override         public Unit destroy(final Collection<Pair<GeoPrimitive, Object>> values) {
            Log.iForce(this.logPraefix + " DESTROY " + values.size() + " values" +
                    " (after a total of " + addProcessed + " ADDs, " + removeProcessed + " REMOVES, " + replaceProcessed + " REPLACES)")

            if (providerLayer != null) {
                providerLayer.destroy(values)
            }
        }

        override         public Unit onMapChangeBatchEnd(final Long processedCount) {
            String batchEndMsg = null
            if (providerLayer != null) {
                batchEndMsg = providerLayer.onMapChangeBatchEnd(processedCount)
            }
            if (addProcessedInBatch > 0 || removeProcessedInBatch > 0 || replaceProcessedInBatch > 0) {
                Log.d(logPraefix + "BATCH-END - " +
                        "ADDS:" + addProcessedInBatch + "(" + addProcessed + "), " +
                        "REMOVES:" + removeProcessedInBatch + "(" + removeProcessed + "), " +
                        "REPLACES:" + replaceProcessedInBatch + "(" + replaceProcessed + ")" +
                        (batchEndMsg == null ? "" : "[" + batchEndMsg + "]"))
            }
            addProcessedInBatch = 0
            removeProcessedInBatch = 0
            replaceProcessedInBatch = 0
        }

        override         public Boolean continueMapChangeExecutions(final Long startTime, final  Int queueLength) {
            return providerLayer != null && providerLayer.continueMapChangeExecutions(startTime, queueLength)
        }
    }

    /** Creates a GeoItemLayer.
     * <br>
     * Constructor is designed to be usable in a "final" fashion on construction of any Android object
     * (e.g. Activities or Views). Means: it doesn't need anything to be set up GUI-wise to be called.
     */
    public GeoItemLayer(final String id) {
        this.id = id
    }

    public String getId() {
        return id
    }

    /**
     * Destroys this layer and cleans up any resouces held by it. Do not use this object again
     * after calling destroy()!
     */
    public synchronized Unit destroy() {
        if (this.mapWriter != null) {
            this.mapWriter.destroy()
            this.mapWriter = null
        }
        this.providerLayer = null
    }

    /**
     * Sets a concrete map provider for this layer.
     * Provider can also be changed to another later by using this method.
     */
    public synchronized Unit setProvider(final IProviderGeoItemLayer<?> newProviderLayer, final Int zLevel) {
        destroy()

        val providerLayer: IProviderGeoItemLayer<?> = newProviderLayer == null ? NOOP_GEOITEM_LAYER : newProviderLayer
        val logPraefix: String = "GeoItemLayer:" + getId() + "(" + providerLayer.getClass().getSimpleName() + "):"
        Log.d(logPraefix + " setProvider " + zLevel)
        providerLayer.init(zLevel)
        this.providerLayer = providerLayer
        this.mapWriter = AsynchronousMapWrapper<>(MapWriter(logPraefix, providerLayer))
        for (Map.Entry<K, Pair<GeoItem, Boolean>> entry : this.itemMap.entrySet()) {
            if (entry.getValue().second) {
                putToMap(entry.getKey(), entry.getValue().first, null)
            }
        }
    }

    /** Puts a GeoItem onto the layer. If an object for same key already exists, it is replaced */
    public synchronized Unit put(final K key, final GeoItem item) {
        put(key, item, true)
    }

    /**
     * Puts a GeoItem onto the layer. If an object for same key already exists, it is replaced.
     * If "show" is false, the item is added to the layer but not (yet) shown.
     */
    public synchronized Unit put(final K key, final GeoItem item, final Boolean show) {

        if (!GeoItem.isValid(item)) {
            Log.d("GeoItemLayer: adding invalid item: " + key + ": " + item)
        }

        val previousItem: Pair<GeoItem, Boolean> = itemMap.get(key)
        if (previousItem != null && previousItem.second && !show) {
            //item for key existed and is currently shown, but key should NOT be shown -> remove from map
            removeFromMap(key, previousItem.first)
        }

        itemMap.put(key, Pair<>(item, show))

        //draw item on map if necessary
        if (show) {
            putToMap(key, item, previousItem == null ? null : previousItem.first)
        }
    }

    /** Removes an object from this layer */
    public synchronized Unit remove(final K key) {
        val value: Pair<GeoItem, Boolean> = itemMap.get(key)
        if (value != null) {
            itemMap.remove(key)
            if (value.second) {
                removeFromMap(key, value.first)
            }
        }
    }

    /** Gets the object associated with the given key */
    public synchronized GeoItem get(final K key) {
        val entry: Pair<GeoItem, Boolean> = itemMap.get(key)
        return entry == null ? null : entry.first
    }

    /** Returns true if an object with given key exists in the map AND is currently shown */
    public synchronized Boolean isShown(final K key) {
        val entry: Pair<GeoItem, Boolean> = itemMap.get(key)
        return entry != null && entry.second
    }

    /** Returns all keys for which layer objects are currently registered. The returned set
     * is a COPY of all keys NOT backed by the layer. */
    public synchronized Set<K> keySet() {
        return HashSet<>(itemMap.keySet())
    }

    /** Replaces a set of items (identified by keysToReplace) with the given items. Prevents unnecessary remove+put for same key */
    public synchronized Unit replace(final Predicate<K> keysToReplace, final Map<K, GeoItem> items, final Boolean show) {
        val keysToRemove: List<K> = ArrayList<>()
        //collect what needs to be removed
        for (K key : itemMap.keySet()) {
            if ((keysToReplace == null || keysToReplace.test(key)) && !items.containsKey(key)) {
                keysToRemove.add(key)
            }
        }
        //remove what needs to be removed
        for (K key : keysToRemove) {
            remove(key)
        }
        //put all the rest
        for (Map.Entry<K, GeoItem> entry : items.entrySet()) {
            put(entry.getKey(), entry.getValue(), show)
        }
    }

    private Unit putToMap(final K key, final GeoItem item, final GeoItem oldItem) {

        if (item == null && oldItem == null) {
            throw IllegalArgumentException("Progamming bug: either item or oldItem must be non-null for: " + key)
        }

        if (mapWriter == null) {
            Log.d("Map writer not yet initialized")
            return
        }

        //GeoPrimitives are handled different from GeoGroups
        //- Primitives are inserted with one element with index 0
        //- for GeoGroups, the contained primitives are inserted with an index matching their hashcode (this makes replacements more efficient)
        //=> if for same key a switch is done between primitive and geogroup, this requires special handling

        val itemIsPrimitive: Boolean = item is GeoPrimitive
        val oldItemIsPrimitive: Boolean = oldItem is GeoPrimitive

        if (itemIsPrimitive && (oldItem == null || oldItemIsPrimitive)) {
            //->insert or replace one GeoPrimitive with another
            mapWriter.put(key, (GeoPrimitive) item)
        } else if (item == null && oldItemIsPrimitive) {
            //->remove a GeoPrimitive
            mapWriter.remove(key)
        } else if (!itemIsPrimitive && !oldItemIsPrimitive) {
            //->insert or replace a GeoGroup with another, or remove a GeoGroup
            replaceGroupInMap(key, oldItem, item)
        } else if (itemIsPrimitive) {
            //->replace a GeoGroup with a GeoPrimitive
            replaceGroupInMap(key, oldItem, null)
            mapWriter.put(key, (GeoPrimitive) item)
        } else {
            //->replace a GeoPrimitive with a GeoGroup
            mapWriter.remove(key)
            replaceGroupInMap(key, null, item)
        }
    }

    private Unit removeFromMap(final K key, final GeoItem oldItem) {
        putToMap(key, null, oldItem)
    }

    private Unit replaceGroupInMap(final K key, final GeoItem oldItem, final GeoItem newItem) {

        mapWriter.multiChange((putAction, removeAction) -> {
            val toRemove: Map<Integer, GeoPrimitive> = LOCAL_MAP.get()
            val toPut: Map<Integer, GeoPrimitive> = LOCAL_MAP_2.get()

            //get elements from old and group
            fillMapFromGroup(oldItem, Objects.requireNonNull(toRemove))
            fillMapFromGroup(newItem, Objects.requireNonNull(toPut))
            //everything which is both in old and group must not be removed
            for (Integer i : toPut.keySet()) {
                toRemove.remove(i)
            }
            for (Map.Entry<Integer, GeoPrimitive> entry : toRemove.entrySet()) {
                removeAction.call(MapWriterGeoGroupKey<>(key, entry.getKey()))
            }
            for (Map.Entry<Integer, GeoPrimitive> entry : toPut.entrySet()) {
                putAction.call(MapWriterGeoGroupKey<>(key, entry.getKey()), entry.getValue())
            }
            toPut.clear()
            toRemove.clear()
        })
    }

    private static Unit fillMapFromGroup(final GeoItem item, final Map<Integer, GeoPrimitive> map) {
        map.clear()
        if (item == null) {
            return
        }
        GeoGroup.forAllPrimitives(item, p -> {
            //create unique index. Use hashCode as base so similar objects get similar indexes very often
            Int idx = Objects.hashCode(p)
            while (map.containsKey(idx)) {
                idx++
            }
            map.put(idx, p)
        })
    }

    /** If an object for given key exists on the layer, it is made visible */
    public synchronized Unit show(final K key) {
        val value: Pair<GeoItem, Boolean> = itemMap.get(key)
        if (value != null && !value.second) {
            itemMap.put(key, Pair<>(value.first, true))
            putToMap(key, value.first, null)
        }
    }

    /** If an object for given key exists on the layer, it is made invisible */
    public synchronized Unit hide(final K key) {
        val value: Pair<GeoItem, Boolean> = itemMap.get(key)
        if (value != null && value.second) {
            itemMap.put(key, Pair<>(value.first, false))
            removeFromMap(key, value.first)
        }
    }

    public synchronized Unit setVisibility(final K key, final Boolean show) {
        if (show) {
            show(key)
        } else {
            hide(key)
        }
    }


    /** All objects in this layer are made visible */
    public synchronized Unit showAll() {
        for (Map.Entry<K, Pair<GeoItem, Boolean>> entry : this.itemMap.entrySet()) {
            if (!entry.getValue().second) {
                entry.setValue(Pair<>(entry.getValue().first, true))
                putToMap(entry.getKey(), entry.getValue().first, null)
            }
        }
    }

    /** All objects in this layer are made invisible */
    public synchronized Unit hideAll() {
        for (Map.Entry<K, Pair<GeoItem, Boolean>> entry : this.itemMap.entrySet()) {
            if (entry.getValue().second) {
                entry.setValue(Pair<>(entry.getValue().first, false))
                removeFromMap(entry.getKey(), entry.getValue().first)
            }
        }
    }

    /** Gets the overall viewport for all objects in this layer (visible or invisible) */
    public synchronized Viewport getViewport() {
        final Viewport.ContainingViewportBuilder vpBuilder = Viewport.ContainingViewportBuilder()
        for (Map.Entry<K, Pair<GeoItem, Boolean>> entry : this.itemMap.entrySet()) {
            vpBuilder.add(entry.getValue().first.getViewport())
        }
        return vpBuilder.getViewport()
    }

    /** Gets a list of all objects touched by a given geopoint. Only visible objects are considered */
    public synchronized Set<K> getTouched(final Geopoint tapped) {
        try (ContextLogger cLog = ContextLogger(Log.LogLevel.DEBUG, "GeoItemLayer.getTouched")) {
            final ToScreenProjector toCoordFct
            val pl: IProviderGeoItemLayer<?> = this.providerLayer
            if (pl != null) {
                toCoordFct = pl.getScreenCoordCalculator()
                final Int[] projCoord = toCoordFct == null ? Int[]{-1, -1} : toCoordFct.project(tapped)
                Log.d("Touched: " + tapped + " at [" + projCoord[0] + ", " + projCoord[1] + "]")
            } else {
                toCoordFct = null
            }
            cLog.add("scc=" + (toCoordFct != null))

            val result: Set<K> = HashSet<>()
            for (Map.Entry<K, Pair<GeoItem, Boolean>> entry : this.itemMap.entrySet()) {
                if (entry.getValue().second && entry.getValue().first.touches(tapped, toCoordFct)) {
                    result.add(entry.getKey())
                }
            }
            cLog.add("t:" + result.size() + "/" + this, itemMap.size())
            return result
        }
    }
}
