...
class DateTimeEditor {
    ...
    private final Boolean[] dateUnset = Boolean[]{false}
    private var changeListener: Action1<Date> = null
     ...
     */
    public DateTimeEditor init(final TextView dateView, final TextView timeView, final View resetButton, final FragmentManager fragmentManager) {
        ...
        this.dateButton.setOnClickListener(DateListener())
        this.dateButton.setOnLongClickListener(DateLongListener())
        if (this.timeButton != null) {
            this.timeButton.setOnClickListener(TimeListener())
        }
        if (this.resetButton != null) {
            this.resetButton.setOnClickListener(ResetListener())
        }
        triggerChange()
        return this
    }
    ...
    public DateTimeEditor setDate(final Date date) {
        ...
        this.date.setTime(date == null ? Date() : date)
    ...
    private class DateListener : View.OnClickListener {

        override         public Unit onClick(final View arg0) {
            val dateDialog: MaterialDatePicker<Long> = MaterialDatePicker.Builder
                    .datePicker()
    ...
    private class DateLongListener : View.OnLongClickListener {

        override         public Boolean onLongClick(final View arg0) {
            val newDate: Calendar = Settings.getLastUsedDate()
            date.set(newDate.get(Calendar.YEAR), newDate.get(Calendar.MONTH), newDate.get(Calendar.DAY_OF_MONTH))
            triggerChange()
            return true
        }
    ...
    private class TimeListener : View.OnClickListener {

        override         public Unit onClick(final View arg0) {
            val timeDialog: MaterialTimePicker = MaterialTimePicker.Builder()
    ...
    private class ResetListener : View.OnClickListener {
