...
class SimpleItemListModel<T> {
    ...
    private var displayIconMapper: Function<T, ImageParam> = (o) -> null

    private var actionIconMapper: Function<T, ImageParam> = (o) -> null

    private var choiceMode: ChoiceMode = ChoiceMode.SINGLE_PLAIN

    private Int[] itemPaddingInDp = Int[]{10, 4, 10, 4}

    private val groupingOptions: GroupingOptions<Object> = GroupingOptions<>()

    private var filterTerm: String = null

    private val selectedItems: Set<T> = HashSet<>()
    private val disabledItems: Set<T> = HashSet<>()

    private final List<Consumer<ChangeType>> changeListeners = ArrayList<>()

    private var actionListener: Consumer<T> = null

    private var columnCount: Int = 1
    private var columnSpanMapper: Function<T, Integer> = null


    /** Supported display modes for choosing items from a list */
    enum class class ChoiceMode { SINGLE_PLAIN, SINGLE_RADIO, MULTI_CHECKBOX }

    /** Types of model changes for which events are fired */
    enum class class ChangeType { COMPLETE, SELECTION, FILTER, GROUP_HEADER }

    /** Specifies options for grouping items */
    class GroupingOptions<G> {

        private var groupMapper: Function<T, G> = null
        private var groupGroupMapper: Function<G, G> = null
        ...
        private var itemGroupComparator: Comparator<Object> = null
        private var groupComparator: Comparator<G> = null
        private var sortGroupsBeforeItems: Boolean = false
        private Predicate<ItemGroup<T, G>> groupPruner = null

        private val reducedGroups: Set<G> = HashSet<>()

        private var reducedGroupSaveId: String = null
        private var reducedGroupIdMapper: Function<G, String> = null
        private var reducedGroupIdBackMapper: Function<String, G> = null
        ...
        public Comparator<Object> getItemGroupComparator() {
            val hasItemGroupComparator: Boolean = itemGroupComparator != null

            val gComparator: Comparator<G> = hasItemGroupComparator ? null : this.groupComparator != null ? this.groupComparator : CommonUtils.getTextSortingComparator(Objects::toString)
            val tComparator: Comparator<T> = hasItemGroupComparator ? null : CommonUtils.getListSortingComparator(null, true, getItems())
            ...
            return CommonUtils.getNullHandlingComparator((o1, o2) -> {
                ...
                val o1IsGroup: Boolean = o1 is ItemGroup
                val o2IsGroup: Boolean = o2 is ItemGroup
        ...
        private Unit loadReducedGroups() {
            if (this.reducedGroupSaveId != null && this.reducedGroupIdBackMapper != null) {
                val node: JsonNode = JsonUtils.stringToNode(Settings.getSimpleListModelConfig(this.reducedGroupSaveId))
                if (node != null) {
                    val groupStrings: List<String> = JsonUtils.getTextList(node, "groups")
                    setReducedGroups(groupStrings.stream().map(s -> this.reducedGroupIdBackMapper.apply(s)).collect(Collectors.toList()))
                }
            }
        }

        private Unit saveReducedGroups() {
   ...
   public SimpleItemListModel<T> setDisplayMapper(final BiFunction<T, ItemGroup<T, Object>, TextParam> displayMapper, final BiFunction<T, ItemGroup<T, Object>, String> textFilterMapper, final BiFunction<Context, ViewGroup, TextView> textViewCreator) {
        if (displayMapper != null) {
    ...
    public static <TT> Func5<TT, ItemGroup<TT, Object>, Context, View, ViewGroup, View> constructDisplayTextViewMapper(final BiFunction<TT, ItemGroup<TT, Object>, TextParam> displayTextMapper, final BiFunction<Context, ViewGroup, TextView> textViewCreator) {
        val tvCreator: BiFunction<Context, ViewGroup, TextView> = textViewCreator != null ? textViewCreator :
            (ctx, parent) -> ViewUtils.createTextItem(ctx, R.style.text_default, TextParam.text(""))
        return (item, itemGroup, context, view, parent) -> {
            val tv: TextView = view is TextView ? (TextView) view : tvCreator.apply(context, parent)
            val tp: TextParam = displayTextMapper == null ? TextParam.text(String.valueOf(item)) : displayTextMapper.apply(item, itemGroup)
            if (tp == null) {
                tv.setText("--")
            } else {
                tp.applyTo(tv)
    ...
    public static <GT, TT> Func5<GT, ItemGroup<TT, GT>, Context, View, ViewGroup, View> constructGroupDisplayViewMapper(final Function<ItemGroup<TT, GT>, TextParam> displayGroupTextMapper) {
        return (group, itemGroup, context, view, parent) -> {
            val tv: TextView = view is TextView ? (TextView) view : ViewUtils.createTextItem(context, R.style.text_default, TextParam.text(""))
            val tp: TextParam = displayGroupTextMapper == null || itemGroup == null ? TextParam.text(String.valueOf(itemGroup)) : displayGroupTextMapper.apply(itemGroup)
            if (tp == null) {
                tv.setText("--")
            } else {
                tp.applyTo(tv)
    ...
    public static <T> BiFunction<T, ItemGroup<T, Object>, String> constructFilterTextExtractor(final BiFunction<T, ItemGroup<T, Object>, TextParam> displayTextMapper) {
        return (item, itemGroup) -> {
            ...
            val tp: TextParam = displayTextMapper == null ? null : displayTextMapper.apply(item, itemGroup)
            if (tp == null) {
     ...
     *
     * @param displayViewMapper needs to return a fully prepared view for a gien item, its group, the previous view (if any) and its viewgroup (for view creation)
    ...
    public SimpleItemListModel<T> setDisplayViewMapper(@LayoutRes final Int layoutResId, final Action3<T, ItemGroup<T, Object>, View> fillView, final BiFunction<T, ItemGroup<T, Object>, String> textFilterMapper) {
        if (layoutResId != 0 && fillView != null) {
            setDisplayViewMapper((item, itemGroup, ctx, view, viewGroup) -> {
                //get or create view
                val realView: View = view == null ? LayoutInflater.from(ctx).inflate(layoutResId, viewGroup, false) : view
                //fill view
    ...
    private Unit pruneSelected() {
        if (choiceMode != ChoiceMode.MULTI_CHECKBOX && selectedItems.size() > 1) {
            val remaining: T = CommonUtils.first(selectedItems)
            selectedItems.clear()
    ...
    private Unit triggerChange(final ChangeType mode) {
