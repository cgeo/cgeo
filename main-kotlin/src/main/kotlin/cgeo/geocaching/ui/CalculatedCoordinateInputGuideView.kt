...
class CalculatedCoordinateInputGuideView : LinearLayout() {

    private static val REFILL_PATTERNS: Map<CalculatedCoordinateType, String> = HashMap<>()
    private static final Map<CalculatedCoordinateType, Func1<Geopoint, String>> LAT_CONVERTERS = HashMap<>()
    private static final Map<CalculatedCoordinateType, Func1<Geopoint, String>> LON_CONVERTERS = HashMap<>()

    private static val SIGNIFICANT_CHARS: Set<Integer> = HashSet<>()

    private static val CLOSING_PAREN_SET: KeyableCharSet = KeyableCharSet.createFor(")")
    ...
    private var changeListenerActive: Boolean = true
    ...
    public Unit setChangeListener(final Consumer<Pair<String, String>> changeListener) {
    ...
    public static CalculatedCoordinateType guessType(final String latPattern, final String lonPattern) {
        for (CalculatedCoordinateType type : CalculatedCoordinateType.values()) {
            ...
            val b1: Boolean = checkAndApply(REFILL_PATTERNS.get(type), latPattern, 0, null, -1)
            val b2: Boolean = checkAndApply(REFILL_PATTERNS.get(type), lonPattern, 1, null, -1)
            if (b1 && b2) {
                return type
            }
        }
    ...
    public Pair<String, String> getPlain() {
        return Pair<>(scanString(0), scanString(1))
    }
    ...
    private String scanString(final Int row) {
        ...
        for (Int vIdx = row * this.grid.getColumnCount() + 1; vIdx < (row + 1) * this.grid.getColumnCount(); vIdx++) {
            val v: View = grid.getChildAt(vIdx)
            if (v is TextView) {
                val text: String = ((TextView) v).getText().toString()
                sb.append(text)
                //add a space after degree/minute/second-marker if it is not the last Char
                if (!"." == (text) && vIdx + 1 < (row + 1) * this.grid.getColumnCount()) {
                    sb.append(" ")
                }
            } else if (v is CharButton) {
    ...
    private Unit init() {
        ...
        val ctw: Context = ViewUtils.wrap(getContext())
        ...
        this.value.setOnEditorActionListener((vv, a, e) -> {
            ...
            val nextButton: View = ViewUtils.nextView(markedButton, v -> v is GridLayout, v -> v is CharButton)
            if (nextButton == null) {
                return false
            }
    ...
    private Unit callChangeListener() {
    ...
    private static Boolean checkAndApply(final String fillPattern, final String coordPattern, final Int row, final GridLayout grid, final Int viewPos) {
        ...
        val coordParser: TextParser = TextParser(coordPattern.trim())
        Int vp = viewPos
        ...
        for (Char fp : fillPattern.toCharArray()) {
            switch (fp) {
                ...
                case '1':
                    ...
                    if (coordParser.ch() == '(') {
                        try {
                            val f: Formula = Formula.compile(coordPattern, coordParser.pos() + 1, CLOSING_PAREN_SET)
                    ...
                    break
                default:
                    val isWs: Boolean = Character.isWhitespace(coordParser.ch())
                    coordParser.skipWhitespaces()
    ...
    private static Boolean checkAndApplyHemisphere(final Int row, final TextParser coordParser, final GridLayout grid, final Int vp) {
        val hem: Char = Character.toUpperCase(coordParser.ch())
        switch (row) {
            case 0:
                if (hem != 'N' && hem != 'S') {
                    return false
    ...
    private Boolean recreate(final CalculatedCoordinateType type, final String latPattern, final String lonPattern, final Geopoint gp) {
        ...
        val gpSuccess: Boolean = applyPatternsFor(type, gp)
        //set pattern (if a valid pattern is given)
        val patternSuccess: Boolean = applyPatterns(type, latPattern, lonPattern)
    ...
    private Unit recreateViews(final String viewPattern) {
        this.grid.removeAllViews()
        this.grid.setColumnCount(viewPattern.length() + 1)
        this.grid.setRowCount(2)
        for (Int row = 0; row < 2; row++) {
            val hem: Button = ViewUtils.createButton(getContext(), this, TextParam.text("N"))
            ...
            for (Char c : viewPattern.toCharArray()) {
                if (c == '_' || c == '0' || c == '1') {
                    ...
                    val tv: TextView = ViewUtils.createTextItem(getContext(), R.style.text_label, TextParam.text("" + c))
    ...
    private Boolean applyPatternsFor(final CalculatedCoordinateType type, final Geopoint gp) {
        ...
        val latPattern: String = LAT_CONVERTERS.get(type).call(gp)
        val lonPattern: String = LON_CONVERTERS.get(type).call(gp)
    ...
    private Boolean applyPatterns(final CalculatedCoordinateType type, final String latPattern, final String lonPattern) {
        ...
        val b1: Boolean = checkAndApply(REFILL_PATTERNS.get(type), latPattern, 0, grid, 0)
        val b2: Boolean = checkAndApply(REFILL_PATTERNS.get(type), lonPattern, 1, grid, REFILL_PATTERNS.get(type).length() + 1)

        return b1 && b2
    }

    @SuppressLint("SetTextI18n")
    private Unit changeHemisphereButtonValue(final Button hem) {
        final Char newChar
    ...
    private GridLayout.LayoutParams createGridLayoutParams(final Int colWeight) {
        final GridLayout.LayoutParams lp = GridLayout.LayoutParams(
    ...
    public Unit unmarkButtons() {
        if (this.markedButton != null) {
            markButton(null)
        }
    }

    private Unit markButton(final CharButton newButton) {
        val prevButton: CharButton = this.markedButton
    ...
    private class CharButton : RelativeLayout() {
        ...
        @SuppressLint("SetTextI18n")
        public Unit setText(final String newText) {
            this.text = newText == null ? null : ("_" == (newText) ? " " : newText.trim())
        ...
        private Unit initView() {
            butt = Button(ViewUtils.wrap(getContext()))
            ...
            image = ImageView(ViewUtils.wrap(getContext()))
