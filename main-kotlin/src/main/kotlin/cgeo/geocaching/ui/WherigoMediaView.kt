...
class WherigoMediaView : LinearLayout() {
    ...
    public WherigoMediaView(final Context context, final AttributeSet attrs) {
        super(context, attrs)
        init(attrs, 0, 0)
    }

    public WherigoMediaView(final Context context, final AttributeSet attrs, final Int defStyleAttr) {
    ...
    private Unit setMediaData(final Int mediaId, final String type, final String fileName, final String altText, final Supplier<Byte[]> dataSupplier) {
        ...
        val cacheDir: File = WherigoGame.get().getCacheDirectory()
        val mediaFile: File = cgeo.geocaching.utils.FileUtils.getOrCreate(cacheDir, "media-" + fileName, type, data)
    ...
    public Unit setMediaData(final String type, final Byte[] data, final String altText) {
        val mediaId: Int = this.mediaId >= 0 ? -1 : this.mediaId - 1
        setMediaData(mediaId, type, "id" + mediaId, altText, () -> data)
    }

    public Unit setMedia(final Media media) {
