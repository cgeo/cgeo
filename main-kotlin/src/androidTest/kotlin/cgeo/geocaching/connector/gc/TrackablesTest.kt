...
class TrackablesTest {

    @Test
    public Unit testTrackable() {
        val trackable: Trackable = getTB2R124()
    ...
    @Test
    public Unit testTrackableWithoutImage() {
        val trackable: Trackable = getTB2R124()
    ...
    @Test
    public Unit testTrackableWithLogImages() {
        val trackable: Trackable = parseTrackable(R.raw.tbxatg)
        assertThat(trackable.getGeocode()).isEqualTo("TBXATG")

        val log: List<LogEntry> = trackable.getLogs()
        assertThat(log).isNotNull()
        assertThat(log).hasSize(10)

        val logImages: List<Image> = log.get(5).logImages
    ...
    @Test
    public Unit testParseTrackableWithoutReleaseDate() {
        val trackable: Trackable = parseTrackable(R.raw.tb14wfv)
    ...
    @Test
    public Unit testParseTrackableWithRetrievedDate() {
        val trackable: Trackable = parseTrackable(R.raw.tb11jzk)
        assertThat(trackable).isNotNull()
    ...
    @Test
    public Unit testParseTrackableWithDiscoveredDate() {
        val trackable: Trackable = parseTrackable(R.raw.tb84bz5)
        assertThat(trackable).isNotNull()
        assertThat(trackable.getLogDate()).isNotNull()
    ...
    @Test
    public Unit testParseRelativeLink() {
        val trackable: Trackable = parseTrackable(R.raw.tb4cwjx)
        assertThat(trackable).isNotNull()
        assertThat(trackable.getName()).isEqualTo("The Golden Lisa")
        val goal: String = trackable.getGoal()
    ...
    private Trackable parseTrackable(final Int trackablePage) {
        val pageContent: String = CgeoTestUtils.getFileContent(trackablePage)
        return GCParser.parseTrackable(TextUtils.replaceWhitespace(pageContent), null)
    }

    @Test
    public Unit testParseMarkMissing() {
        val trackable: Trackable = parseTrackable(R.raw.tb3f206)
        assertThat(trackable).isNotNull()
        val logs: List<LogEntry> = trackable.getLogs()
        assertThat(logs).isNotNull()
        assertThat(logs).isNotEmpty()
        val marked: LogEntry = logs.get(0)
    ...
    @Test
    public Unit testParseTrackableNotExisting() {
        val trackable: Trackable = GCParser.parseTrackable(CgeoTestUtils.getFileContent(R.raw.tb_not_existing), null)
