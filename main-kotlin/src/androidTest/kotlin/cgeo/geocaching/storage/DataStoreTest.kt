...
import cgeo.geocaching.enumerations.LoadFlags.REMOVE_ALL
...
class DataStoreTest {
    ...
    static val ARTIFICIAL_GEOCODE: String = "TEST"

    private static val MILLISECONDS_PER_DAY: Long = 1000 * 60 * 60 * 24

    @Test
    public Unit testStoredLists() {
        ...
        try {
            ...
            // get list
            val list1: StoredList = DataStore.getList(listId1)
    ...
    public Unit testLoadWaypoints() {
        val viewport: Viewport = Viewport(Geopoint(-1, -2), Geopoint(3, 4))
        DataStore.loadWaypoints(viewport)
    }

    // Check that saving a cache and trackable without logs works (see #2199)
    @Test
    public Unit testSaveWithoutLogs() {
        ...
        val inventory: List<Trackable> = ArrayList<>()
        inventory.add(trackable)
        cache.setInventory(inventory)

        try {
            DataStore.saveCache(cache, EnumSet.of(SaveFlag.DB))
            val loadedCache: Geocache = DataStore.loadCache(ARTIFICIAL_GEOCODE, LoadFlags.LOAD_ALL_DB_ONLY)
    ...
    @Test
    public Unit testGeocodeCaseInsensitive() {

        val upperCase: String = ARTIFICIAL_GEOCODE
        val lowerCase: String = StringUtils.lowerCase(upperCase)
        ...
        try {
            val oldCache: Geocache = DataStore.loadCache(upperCase, LoadFlags.LOAD_ALL_DB_ONLY)
            assertThat(oldCache).as("Old cache").isNull()

            DataStore.saveCache(cache, EnumSet.of(SaveFlag.DB))
            val cacheWithOriginalCode: Geocache = DataStore.loadCache(upperCase, LoadFlags.LOAD_ALL_DB_ONLY)
            assertThat(cacheWithOriginalCode).overridingErrorMessage("Cache was not saved correctly!").isNotNull()

            val cacheLowerCase: Geocache = DataStore.loadCache(lowerCase, LoadFlags.LOAD_ALL_DB_ONLY)
    ...
    @Test
    public Unit testLoadLogsFromEmptyGeocode() {
        val logs: List<LogEntry> = DataStore.loadLogs("")
    ...
    public Unit testLog() {
        ...
        logs.add(LogEntry.Builder().setDate(Date().getTime() - MILLISECONDS_PER_DAY * 3).setLog("testlog").setLogType(LogType.NOTE).setServiceLogId("pid").build())
        logs.add(LogEntry.Builder().setDate(Date().getTime() - MILLISECONDS_PER_DAY * 2).setLog("testlog2").setLogType(LogType.NOTE).build())
        DataStore.saveLogs(ARTIFICIAL_GEOCODE, logs, true)

        val logsLoadeded: List<LogEntry> = DataStore.loadLogs(ARTIFICIAL_GEOCODE)

        assertThat(logsLoadeded).containsExactlyInAnyOrderElementsOf(logs)
    }

    @Test
    public Unit testLoadCacheHistory() {
        val history: SearchResult = DataStore.getBatchOfStoredCaches(null, PseudoList.HISTORY_LIST.id)
    ...
    @Test
    public Unit testOfflineLog() {
        val geocode: String = ARTIFICIAL_GEOCODE + "-O"
        val logDate: Date = Date(Date().getTime() - MILLISECONDS_PER_DAY * 3)
        //ensure that we don't overwrite anything in database before starting this test
        DataStore.clearLogOffline(geocode)
        val logEntry: OfflineLogEntry = DataStore.loadLogOffline(geocode)
        ...
        try {
            final OfflineLogEntry.Builder builder = OfflineLogEntry.Builder()
                    ...
                    .setFavorite(false)
                    .setRating(4.5f)
                    .addLogImage(Image.Builder().setUrl("https://www.cgeo.org/images/logo.png").setTitle("The logo").setDescription("This is the logo").build())
                    .addLogImage(Image.Builder().setUrl("https://manual.cgeo.org/_media/type_multi.png").setTitle("Multicache icon").setDescription("This is the icon for a multicache").build())
            ...
            val logId: Int = loadedLogEntry.id
            ...
            builder.setDate(logDate.getTime())
                    ...
                    .setRating(1.0f)
                    .setPassword("pwd")
                    .setLogImages(ArrayList<>())
                    .addLogImage(Image.Builder().setUrl("https://www.cgeo.org/images/logo.png").setTitle("The logo").setDescription("This is the logo").build())
                    .addLogImage(Image.Builder().setUrl("https://manual.cgeo.org/_media/type_virtual.png").setTitle("Virtual icon").setDescription("This is the icon for a virtualcache").build())
                    .addLogImage(Image.Builder().setUrl("https://manual.cgeo.org/_media/type_tradi.png").setTitle("Tradicache icon").setDescription("This is the icon for a traditionalcache").build())
    ...
    @Test
    public Unit testEmptyOfflineLog() {
        val geocode: String = ARTIFICIAL_GEOCODE + "-O"
        ...
        try {
            ...
            val loadedLogEntry: OfflineLogEntry = DataStore.loadLogOffline(geocode)
    ...
    @Test
    public Unit testUpdateOnlineCache() {
        try {
            val cacheOld: Geocache = Geocache()
            cacheOld.setGeocode(ARTIFICIAL_GEOCODE)
            cacheOld.setCoords(Geopoint(1, 2))
            ...
            // update cache-data
            val cacheNew: Geocache = Geocache()
            ...
            val cacheDb: Geocache = DataStore.loadCache(ARTIFICIAL_GEOCODE, LoadFlags.LOAD_CACHE_OR_DB)
    ...
    @Test
    public Unit testUpdateOfflineCache() {
        try {
            val gp: Geopoint = Geopoint(1, 2)
            val cacheOld: Geocache = Geocache()
            cacheOld.setGeocode(ARTIFICIAL_GEOCODE)
            cacheOld.setCoords(Geopoint(1, 2))
            cacheOld.setDescription("Cache description old")
    ...
    private static Unit assertEqualToBuilder(final OfflineLogEntry dbLogEntry, final OfflineLogEntry.Builder builder) {
        val expectedLogEntry: OfflineLogEntry = builder.build()
        ...
        val dbImages: List<Image> = ArrayList<>(dbLogEntry.logImages)
        val expImages: List<Image> = ArrayList<>(expectedLogEntry.logImages)
        //ensure that the order of images is determined for valid content assertion: sort by title
        val imgComp: Comparator<Image> = (image, image2) -> image.getTitle().compareTo(image2.getTitle())
