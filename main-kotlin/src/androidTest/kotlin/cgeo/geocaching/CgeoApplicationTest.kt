 ...
 */
class CgeoApplicationTest {

//    @Test
//    public Unit createTestCaches() {
//        val listId: Int = DataStore.createList("manycaches2");
//        CgeoTestUtils.generateTestCaches(Collections.singleton(listId), "Test", Geopoint(40, 10), 20000);
//    }

    @Test
    @MediumTest
    public Unit testRegEx() {
        val page: String = MockedCache.readCachePage("GC2CJPF")
    ...
    private Unit testUsername(final String username) {
        // pagePrefix / pagePostfix is part of the surrounding JS code around the username parameter
        val pagePrefix: String = "\"publicGuid\":\"abcdef01-2345-6789-abcd-ef0123456789\",\"referenceCode\":\"PRABCDE\",\"id\":1234567,\"username\":\""
        val pagePostfix: String = "\",\"dateCreated\":\"2010-01-01T00:00:00\""
        val purifiedUsername: String = StringEscapeUtils.unescapeEcmaScript(username); // unescape JS-escaped Unicode characters
    ...
    @Test
    @MediumTest
    public Unit testSearchTrackableNotExisting() {
        val tb: Trackable = GCParser.searchTrackable("123456", null, null)
    ...
    @Test
    @MediumTest
    public Unit testSearchTrackable() {
        val trackable: String = "TB2J1VZ"
        val msgPrefix: String = "Trackable " + trackable + ":"
        val tb: Trackable = GCParser.searchTrackable(trackable, null, null)
        ...
        assertThat(tb.getReleased()).as(msgPrefix + "released").isEqualTo(GregorianCalendar(2009, 8 - 1, 24).getTime())
        ...
        for (final LogEntry log : tb.getLogs()) {
            val logMsgPrefix: String = msgPrefix + "Log[" + log + "]:"
    ...
    @Test
    @MediumTest
    public Unit testSearchTrackableSpottedLogState() {
        val tb: Trackable = GCParser.searchTrackable("TB4BPQK", null, null)
        ...
        val lastLog: LogEntry = tb.getLogs().get(0)
        assertThat(lastLog.author).isEqualTo("cachertimsi")
        assertThat(lastLog.date).isEqualTo(GregorianCalendar(2013, 11 - 1, 5).getTimeInMillis())
    ...
    public Geocache searchByGeocode(final String geocode) {
        val search: SearchResult = Geocache.searchByGeocode(geocode, null, true, null)
    ...
    @Test
    @MediumTest
    public Unit testSearchByGeocodeNotExisting() {
        val search: SearchResult = Geocache.searchByGeocode("GC1", null, true, null)
        assertThat(search).isNotNull()
     ...
     */
    private static Unit withMockedLoginDo(final Runnable runnable) {
        val credentials: Credentials = Settings.getGcCredentials()
        val memberStatus: GCMemberState = Settings.getGCMemberStatus()
    ...
    @MediumTest
    @Test
    public Unit testSearchByGeocodeNotLoggedIn() {
        withMockedLoginDo(() -> {
            // non premium cache
            MockedCache cache = GC3FJ5F()
     ...
     */
    private static Unit withMockedFilters(final Runnable runnable) {
        runnable.run()
    }

    @MediumTest
    @Test
    public Unit testSearchByCoords() {

        val distanceFilter: DistanceGeocacheFilter = (DistanceGeocacheFilter) GeocacheFilterType.DISTANCE.create()
        distanceFilter.setCoordinate(Geopoint("N 50° 06.654 E 008° 39.777"))

        val filter: GeocacheFilter = GeocacheFilter.createEmpty().and(distanceFilter)
        val search: SearchResult = GCMap.searchByFilter(GCConnector.getInstance(), filter, GeocacheSort())
    ...
    @MediumTest
    @Test
    public Unit testSearchByOwner() {

        val ownerFilter: OwnerGeocacheFilter = (OwnerGeocacheFilter) GeocacheFilterType.OWNER.create()
        ownerFilter.getStringFilter().setTextValue("Lineflyer")

        val filter: GeocacheFilter = GeocacheFilter.createEmpty().and(ownerFilter)
        val search: SearchResult = GCMap.searchByFilter(GCConnector.getInstance(), filter, GeocacheSort())
    ...
    @MediumTest
    @Test
    public Unit testSearchByUsername() {

        val foundByFilter: LogEntryGeocacheFilter = (LogEntryGeocacheFilter) GeocacheFilterType.LOG_ENTRY.create()
        foundByFilter.setFoundByUser("blafoo")

        val filter: GeocacheFilter = GeocacheFilter.createEmpty().and(foundByFilter)
        val search: SearchResult = GCMap.searchByFilter(GCConnector.getInstance(), filter, GeocacheSort())
    ...
    @MediumTest
    @Test
    public Unit testSearchByViewport() {
        withMockedFilters(() -> {

            val mockedCache: GC3FJ5F = GC3FJ5F()
            CgeoTestUtils.removeCache(mockedCache.getGeocode())

            val viewport: Viewport = Viewport(mockedCache, 0.003, 0.003)

            // check coords
            val search: SearchResult = ConnectorFactory.searchByViewport(viewport, null)
            assertThat(search).isNotNull()
            assertThat(search.getGeocodes()).contains(mockedCache.getGeocode())
            val parsedCache: Geocache = DataStore.loadCache(mockedCache.getGeocode(), LoadFlags.LOAD_CACHE_OR_DB)
    ...
    @MediumTest
    @Test
    public Unit testSearchByGeocodeBasis() {
        for (final MockedCache mockedCache : MockedCache.MOCKED_CACHES) {
            val oldUser: String = mockedCache.getMockedDataUser()
            try {
                mockedCache.setMockedDataUser(Settings.getUserName())
                val parsedCache: Geocache = CgeoApplicationTest().searchByGeocode(mockedCache.getGeocode())
    ...
    @MediumTest
    @Test
    public Unit testSearchByGeocodeSpecialties() {
        val gcv2r9: Geocache = CgeoApplicationTest().searchByGeocode("GCV2R9")
        assertThat(gcv2r9).isNotNull()
        assertThat(gcv2r9.getLocation()).isEqualTo("California, United States")

        val gc1zxez: Geocache = CgeoApplicationTest().searchByGeocode("GC1ZXEZ")
     ...
     */
    private Unit deleteCacheFromDBAndLogout(final String geocode) {
        CgeoTestUtils.removeCache(geocode)

        GCLogin.getInstance().logout()
        // Modify login data to avoid an automatic login again
        TestSettings.setLogin(Credentials("c:geo", "c:geo"))
