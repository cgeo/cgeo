...
class GeocacheFilterTest {
    ...
    @Test
    public Unit emptyFilterWithName() {
    ...
    @Test
    public Unit emptyNameWithFilter() {
        assertFilterFromConfig("name", "", NameGeocacheFilter.class)
    }

    @Test
    public Unit bothfilled() {
    ...
    private GeocacheFilter assertFilterFromConfig(final String config, final String expectedName, final Class<? : IGeocacheFilter()> expectedFilterClass) {
    ...
    @Test
    public Unit legacyParsing() throws ParseException {
        val filterConfig: String = "[=:inconclusive=false:advanced=true]AND(rating:2.0:-;NOT(OR(status:has_user_defined_waypoints_yes:solved_mystery_yes;name:test:contains));difficulty_terrain_matrix:1.0-1.0:1.5-2.5:2.0-2.0:1.5-2.0:include-wo-dt=true)"
        val filter: GeocacheFilter = LegacyFilterConfig.parseLegacy(null, filterConfig, true)

        val json: String = filter.toConfig()
        val filter2: GeocacheFilter = GeocacheFilter.createFromConfig(json)
        val config2: String = filter2.toConfig()
    ...
    @Test
    public Unit genericEmptyFilter() throws ParseException {

        val filterTree: IGeocacheFilter = AndGeocacheFilter()
        for (GeocacheFilterType type : GeocacheFilterType.values()) {
            filterTree.addChild(type.create())
        }

        val filter: GeocacheFilter = GeocacheFilter.create("test", false, false, filterTree)
        assertFilterConfig(filter)
        assertThat(filter.getName()).isEqualTo("test")
    }

    @Test
    public Unit genericFilledFilter() throws ParseException {
        ...
        val filter: GeocacheFilter = GeocacheFilter.create("test", true, true, filterTree)
        assertFilterConfig(filter)
        assertThat(filter.getName()).isEqualTo("test")
    }


    private static Unit assertFilterConfig(final GeocacheFilter filter) throws ParseException {
        val filterJson: String = filter.toConfig()

        val filterFromJson: GeocacheFilter = GeocacheFilter.createFromConfig(filterJson)
    ...
    private static IGeocacheFilter getFilledInstance(final GeocacheFilterType type) {
        val filter: IGeocacheFilter = type.create()
        switch (type) {
            case TYPE:
            ...
            case STATUS:
                ...
                break
            case ATTRIBUTES:
                val atts: Map<CacheAttribute, Boolean> = HashMap<>()
            ...
            case HIDDEN:
            case EVENT_DATE:
                ((HiddenGeocacheFilter) filter).setMinMaxDate(Date(123), Date(456))
            ...
            case LAST_FOUND:
                ...
                ((LastFoundGeocacheFilter) filter).setMinMaxDate(Date(123), Date(456))
                break
            case LOG_ENTRY:
            ...
            case ORIGIN:
                ...
                break
            case STORED_SINCE:
                ((StoredSinceGeocacheFilter) filter).setMinMaxDate(null, Date(1000))
                break
            case CATEGORY:
