...
abstract class Compare {
    ...
    public static Unit assertCompareCaches(final Geocache expected, final Geocache actual, final Boolean all) {
        val geocode: String = expected.getGeocode()
        val cacheStr: String = "Cache " + geocode + ": "
        ...
        val hiddenDate: Date = actual.getHiddenDate()
        ...
        if (all) {
            ...
            assertThat(StringUtils == (expected.getHint(), actual.getHint()) || StringUtils == (expected.getHint(), CryptUtils.rot13(actual.getHint()))).isTrue()
            ...
            for (final Waypoint expectedWpt : expected.getWaypoints()) {
                val actualWpt: Waypoint = actual.getWaypointByPrefix(expectedWpt.getPrefix())
