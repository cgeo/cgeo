...
public class WherigoPushDialogProvider implements IWherigoDialogProvider {
    ...
    public Dialog createAndShowDialog(final Activity activity, final IWherigoDialogControl control) {
        ...
        final AlertDialog dialog = WherigoViewUtils.createFullscreenDialog(activity, LocalizationUtils.getString(cgeo.geocaching.R.string.wherigo_player), binding.getRoot());
    ...
    private void refreshGui(final WherigoThingDetailsBinding binding, final IWherigoDialogControl control) {
        ...
        final String button1Text = button1 != null ? button1 : LocalizationUtils.getString(cgeo.geocaching.R.string.ok);
        final String button2Text = button2;

        WherigoViewUtils.setViewActions(options, binding.dialogActionlist, button2 == null ? 1 : 2, item -> TRUE.equals(item) ?
            TextParam.text(button1Text).setImage(button1 == null ? ImageParam.id(cgeo.geocaching.R.drawable.ic_menu_done) : null) :
