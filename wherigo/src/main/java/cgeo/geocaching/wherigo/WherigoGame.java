...
import cgeo.geocaching.unifiedmap.layers.WherigoZoneInfo;
...
public class WherigoGame implements UI {
    ...
    private WherigoGame() {
        ...
        WherigoZoneProvider.register(this::snapshotZonesForMap, this::isPlaying,
                (activity, zoneName) -> {
                    final Zone zone = getZone(zoneName);
                    if (zone != null) {
                        WherigoViewUtils.displayThing(activity, zone, false);
                    }
                },
                activity -> WherigoViewUtils.getQuickViewDialog(activity).show());

        WherigoInfoBarProvider.register(WherigoInfoBarView::new);
    }

    /** Rendering-only snapshot of the currently visible zones, handed to the base app's map via {@link WherigoZoneProvider}. */
    @NonNull
    private List<WherigoZoneInfo> snapshotZonesForMap() {
        final List<WherigoZoneInfo> result = new ArrayList<>();
        for (final Zone zone : getZones()) {
            if (zone == null || (!isDebugModeForCartridge() && !WherigoUtils.isVisibleToPlayer(zone))) {
                continue;
            }
            final int color = WherigoUtils.isVisibleToPlayer(zone) ? Color.RED : (zone.isActive() ? Color.YELLOW : Color.GRAY);
            final List<Geopoint> geopoints = GP_CONVERTER.fromList(Arrays.asList(zone.points));
            if (!geopoints.isEmpty()) {
                result.add(new WherigoZoneInfo(zone.name, geopoints, color));
            }
        }
        return result;
    }
    ...
    public void notifyListeners(final NotifyType type) {
        ...
        if (type == NotifyType.START || type == NotifyType.REFRESH || type == NotifyType.END) {
            WherigoZoneProvider.notifyChanged();
        }
    }
    ...
    public void setStatusText(final String s) {
        runOnUi(() -> ActivityMixin.showApplicationToast(LocalizationUtils.getString(cgeo.geocaching.R.string.wherigo_short) + ": " + s));
    }
