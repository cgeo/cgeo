...
public enum WherigoLib implements JavaFunction {
    ...
    WherigoLib(final String luaName) {
        this(luaName, WherigoLib.class);
    }
    ...
    private int made (final LuaCallFrame callFrame, final int nArguments) {
        ...
        } catch (final ClassCastException e) { throw new IllegalStateException("bad arguments to object:made"); }
    }

    private int constructNew (final LuaCallFrame callFrame, final int nArguments) {
        try {
    ...
    private int zonePoint (final LuaCallFrame callFrame, final int nArguments) {
        if (nArguments == 0) {
            ...
            final double a = LuaState.fromDouble(callFrame.get(0));
            final double b = LuaState.fromDouble(callFrame.get(1));
            double c = 0;
    ...
    private int distance (final LuaCallFrame callFrame, final int nArguments) {
        final double a = LuaState.fromDouble(callFrame.get(0));
        final String b = (String)callFrame.get(1);
        final double dist = ZonePoint.convertDistanceFrom(a, b);
    ...
    private int distanceGetValue (final LuaCallFrame callFrame, final int nArguments) {
        final double a = LuaState.fromDouble(callFrame.get(0));
        final String b = (String)callFrame.get(1);
        final double dist = ZonePoint.convertDistanceTo(a, b);
        callFrame.push(LuaState.toDouble(dist));
        return 1;
    }
    ...
    private int dialog (final LuaCallFrame callFrame, final int nArguments) {
        ...
        for (int i = 1; i <= n; i++) {
            final LuaTable item = (LuaTable)lt.rawget(LuaState.toDouble(i));
    ...
    private int nocaseequals (final LuaCallFrame callFrame, final int nArguments) {
        final Object a = callFrame.get(0); final Object b = callFrame.get(1);
    ...
    private int showscreen (final LuaCallFrame callFrame, final int nArguments) {
        final int screen = (int)LuaState.fromDouble(callFrame.get(0));
        EventTable et = null;
        if (nArguments > 1) {
    ...
    private int translatePoint (final LuaCallFrame callFrame, final int nArguments) {
        ...
        final double dist = LuaState.fromDouble(callFrame.get(1));
        final double angle = LuaState.fromDouble(callFrame.get(2));
    ...
    private int vectorToPoint (final LuaCallFrame callFrame, final int nArguments) {
        ...
        final double bearing = ZonePoint.angle2azimuth(b.bearing(a));
        final double distance = b.distance(a);
