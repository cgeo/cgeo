...
public class Engine implements Runnable {
    ...
    protected Engine (final CartridgeFile cf, final OutputStream out) throws IOException {
        gwcfile = cf;
    ...
    public static void message (final LuaTable message) {
        final String[] texts = {removeHtml((String)message.rawget("Text"))};
        log("CALL: MessageBox - " + texts[0].substring(0, Math.min(100,texts[0].length())), LOG_CALL);
        final Media[] media = {(Media)message.rawget("Media")};
        String button1 = null, button2 = null;
        final LuaTable buttons = (LuaTable)message.rawget("Buttons");
        if (buttons != null) {
            button1 = (String)buttons.rawget(LuaState.toDouble(1));
            button2 = (String)buttons.rawget(LuaState.toDouble(2));
        }
    ...
    public static void dialog (final String[] texts, final Media[] media) {
        if (texts.length > 0) {
    ...
    /** calls input to UI */
    public static void input (final EventTable input) {
    ...
    private static void replace (final String source, final String pattern, final String replace, final StringBuffer builder) {
        int pos = 0;
        final int pl = pattern.length();
        builder.delete(0, builder.length());
        while (pos < source.length()) {
            final int np = source.indexOf(pattern, pos);
            if (np == -1) break;
     ...
     */
    public static String removeHtml (final String s) {
        if (s == null) return "";
