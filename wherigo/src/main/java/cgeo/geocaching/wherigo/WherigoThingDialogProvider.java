...
import cgeo.geocaching.wherigo.databinding.WherigoThingDetailsBinding;
...
public class WherigoThingDialogProvider implements IWherigoDialogProvider {
    ...
    private enum ThingAction {
        DISPLAY_ON_MAP(TextParam.id(cgeo.geocaching.R.string.caches_on_map).setAllCaps(true).setImage(ImageParam.id(cgeo.geocaching.R.drawable.ic_menu_mapmode))),
        COMPASS(TextParam.id(R.string.wherigo_zone_navigate_compass).setAllCaps(true).setImage(ImageParam.id(cgeo.geocaching.R.drawable.ic_menu_compass))),
        COPY_CENTER(TextParam.id(R.string.wherigo_zone_copy_coordinates).setAllCaps(true).setImage(ImageParam.id(cgeo.geocaching.R.drawable.ic_menu_copy))),
        LOCATE_ON_CENTER(TextParam.id(R.string.wherigo_locate_on_center).setAllCaps(true).setImage(ImageParam.id(cgeo.geocaching.R.drawable.map_followmylocation_btn))),
    ...
    private void refreshActionList(final Activity activity, final IWherigoDialogControl control, final WherigoThingDetailsBinding binding) {
        ...
        WherigoViewUtils.setViewActions(actions, binding.dialogActionlist, 1,
                a -> a instanceof Action ?
                    TextParam.text(WherigoUtils.getUserDisplayableActionText((Action) a)).setImage(ImageParam.id(cgeo.geocaching.R.drawable.settings_nut)) :
                ...
                item -> {
                    ...
                    switch (thingAction) {
                        ...
                        case COPY_CENTER:
                            ...
                            ViewUtils.showShortToast(activity, cgeo.geocaching.R.string.clipboard_copy_ok);
                            break;
