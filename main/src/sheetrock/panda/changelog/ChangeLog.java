...
public class ChangeLog {
    ...
    // use this tag, if the last version is unknown - this happens when the app is first started. If you then want
    // to show a change log up to a certain version tag the next one also as "$ version_unknown"
    private static final String TAG_VERSION_UNKNOWN = "version_unknown";
    ...
    public ChangeLog(Context context, SharedPreferences sp) {
        this(context, sp.getString(VERSION_KEY, ""));

        // save new version name to preferences
     ...
     * Constructor
     *
     * This constructor doesn't touch the shared preferences of the project. Take care of retreiving
     * the lastVersion and storing the actual version (getThisVersion) yourself.
     *
     * @param context
     * @param lastVersion
     *            the version of the app when it was last started
     * @see getThisVersion
     */
    public ChangeLog(Context context, String lastVersion) {
        this.context = context;

        this.lastVersion = lastVersion;
        if (this.lastVersion == null || this.lastVersion.length() == 0) {
            this.lastVersion = TAG_VERSION_UNKNOWN;
        }
