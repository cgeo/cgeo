...
public class RenderThemeLegend {
    ...
    public static void showLegend(final Activity activity, final RenderThemeHelper rth, final DisplayModel displayModel) {
        ...
        for (LegendCategory cat : categories) {
            ...
            final int count1 = fillGrid(activity, entries, grid1, cat.columns1, cat.id1, rth, displayModel);

            final WrappingGridView grid2 = v.findViewById(R.id.legend_grid2);
            final int count2 = fillGrid(activity, entries, grid2, cat.columns2, cat.id2, rth, displayModel);
    ...
    private static int fillGrid(final Activity activity, final ArrayList<LegendEntry> entries, final WrappingGridView grid, final int columns, final int id, final RenderThemeHelper rth, final DisplayModel displayModel) {
        ...
        grid.setAdapter(new LegendGridAdapter(activity, a, rth, displayModel));
    ...
    private static class LegendGridAdapter extends BaseAdapter {
        ...
        LegendGridAdapter(final Activity activity, final ArrayList<LegendEntry> entries, final RenderThemeHelper rth, final DisplayModel displayModel) {
        ...
        public View getView(final int position, final View convertView, final ViewGroup parent) {
            ...
            if (entry.drawable == 0) {
                if (resourceProvider != null) {
                    try {
                        im.setImageBitmap(AndroidGraphicFactory.getBitmap(XmlUtils.createBitmap(AndroidGraphicFactory.INSTANCE, displayModel, entry.relativePath, entry.filename, resourceProvider, 0, height, 105)));
