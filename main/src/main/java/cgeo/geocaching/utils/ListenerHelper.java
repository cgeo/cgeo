...
public class ListenerHelper<T> {
    ...
    public void execute(@Nullable final Scheduler scheduler, final Consumer<T> action) {
        executeWithRemove(scheduler, t -> {
            action.accept(t);
            return false;
        });
    }

    public void executeWithRemove(@Nullable final Scheduler scheduler, final Predicate<T> action) {
        if (action == null) {
            return;
        }
