...
public class TranslatorUtils {
    ...
    public static void translateAny(final BiConsumer<String, Consumer<String>> translator, final Map<String, String> cache, final String textOrHtml, final Consumer<String> consumer) {

        //if possible, return from cache
        final String cacheTranslated = cache == null ? null : cache.get(textOrHtml);
        if (cacheTranslated != null) {
            AndroidRxUtils.runOnUi(() -> consumer.accept(cacheTranslated));
            return;
        }
        final Consumer<String> cacheAwareConsumer = t -> {
            if (cache != null) {
                cache.put(textOrHtml, t);
            }
            consumer.accept(t);
        };
        ...
        if (StringUtils.isBlank(textOrHtml)) {
            ...
            translateHtml(translator, textOrHtml, cacheAwareConsumer);
        } else {
            translator.accept(textOrHtml, cacheAwareConsumer);
        }
    }
    ...
    public static Disposable initializeView(final String id, final Context context, final Translator translator,
                                            final Button button, final View box, final TextView status, final boolean forceVisible) {

        final Consumer<Boolean> circularSetter = ViewUtils.createCircularProgressSetter(button);

        //changes on state
        final Disposable disposable = translator.addStateListener((s, t) -> {
            Log.iForce(LOGPRAEFIX + "Got state: " + s + " (" + id + ", " + t + ")");
            //text
            if (status == null) {
                button.setText(getUserDisplayableStatusShort(t));
            } else {
                status.setText(getUserDisplayableStatusLong(t));
            }
            //circular on/off
            circularSetter.accept(s != TranslatorState.READY && s != TranslatorState.ERROR &&
                s != TranslatorState.TRANSLATED);
            //visibility
            final View visibilityView = box != null ? box : button;
            final String srcLng = t.getSourceLanguage();
            final String detSrcLng = t.getSourceLanguageDetected();
            visibilityView.setVisibility(forceVisible || srcLng != null ||
                (detSrcLng != null && !Settings.getLanguagesToNotTranslate().contains(detSrcLng))
        ...
        button.setOnClickListener(v -> {
            final TranslatorState s = translator.getState();
            if (s != TranslatorState.READY && s != TranslatorState.ERROR && s != TranslatorState.TRANSLATED) {
                changeSettings(context, translator);
    ...
    private static void changeSettings(final Context context, final Translator translator) {
        ...
        final Consumer<Boolean> clickAction = enable -> {
            translator.set(sourceLanguage.get(), targetLanguage.get(), enable);
        };

        if (translator.isEnabled()) {
            dialog.setNeutralButton(TextParam.text("Turn Off"))
                .setNeutralAction(() -> translator.set(sourceLanguage.get(), targetLanguage.get(), false));
        }
    ...
    private static void enableTranslation(final Context context, final Translator translator, final String sourceLanguage, final String targetLanguage) {
        ...
        if (sourceNeedsDownload || targetNeedsDownload) {
            ...
            SimpleDialog.ofContext(context).setTitle(R.string.translator_model_download_confirm_title)
                ...
                .confirm(() -> {
                    TranslationModelManager.get().downloadLanguage(sourceLanguage);
                    TranslationModelManager.get().downloadLanguage(targetLanguage);
                    translator.set(sourceLanguage, targetLanguage, true);
                });
    ...
    public static CharSequence getUserDisplayableStatusLong(final Translator t) {
        ...
        final String status;
        switch (t.getState()) {
            case SOURCE_LANGUAGE_DETECTED:
                status = "awaiting model download";
                break;
            case ERROR:
