...
public class TranslatorUtils {
    ...
    public static void translateAny(final BiConsumer<String, Consumer<String>> translator, final Map<String, String> cache, final String textOrHtml, final Consumer<String> consumer) {
        ...
        final BiConsumer<String, Consumer<String>> cleanedTranslator = (source, c) -> {
          if (StringUtils.isBlank(source)) {
              AndroidRxUtils.runOnUi(() -> c.accept(source));
          } else {
              translator.accept(source, c);
          }
        };
    ...
    private static void translateParagraphedText(final BiConsumer<String, Consumer<String>> translator, final String text, final Consumer<String> consumer) {
        ...
        if (remaining.get() == 0) {
            //Nothing to translate
            AndroidRxUtils.runOnUi(() -> consumer.accept(text));
            return;
        }
    ...
    public static Disposable initializeView(final String id, final Context context, final Translator translator,
        ...
        final Disposable disposable = translator.addStateListener((s, t) -> {
            ...
            circularSetter.accept(
                s == TranslatorState.REINITIALIZED || s == TranslatorState.TRANSLATING ||
                s == TranslatorState.SOURCE_LANGUAGE_DETECTED_MODEL_NEEDED);
            //visibility
    ...
    public static CharSequence getUserDisplayableStatusLong(final Translator t) {
        ...
        final String status;
        switch (t.getState()) {
            case SOURCE_LANGUAGE_DETECTED_MODEL_NEEDED:
