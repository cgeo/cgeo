 ...
 * need to be supported: CREATED (initial) ->  SOURCE_LANGUAGE_DETECTED -> READY (and ERROR in case of
 * problems).
 * <br>
...
class TranslatorImplementation {
    ...
    private BiConsumer<TranslatorState, TranslatorImplementation> stateListener;
    ...
    TranslatorImplementation(final String sourceLanguage, final String targetLanguage, final String sourceDetectionText, final BiConsumer<TranslatorState, TranslatorImplementation> stateListener) {
        ...
        executeOnWorker(() -> {
            findSourceLanguage(sourceLanguage, sourceDetectionText, srcLng -> {
                ...
                changeState(TranslatorState.SOURCE_LANGUAGE_DETECTED);
    ...
    public void translate(final String original, final Consumer<String> onTranslated) {
        ...
        if (translator == null) {
            ...
            executeOnMain(() -> onTranslated.accept(null));
            return;
        }

        translator.translate(original).addOnCompleteListener(getWorkerExecutor(), task -> {
            if (task.getException() != null) {
                Log.w(LOGPRAEFIX + "Error on translating '" + original + "' [" + this + "]", task.getException());
            }
            executeOnMain(() -> onTranslated.accept(task.getResult()));
        });
    }
    ...
    public void dispose() {
        ...
        synchronized (mutex) {
            this.state = TranslatorState.ERROR;
            stateListener = null;
        }
    }
    ...
    private void executeOnMain(final Runnable runnable) {
    ...
    private void changeState(final TranslatorState state) {
        synchronized (mutex) {
            if (this.state == state) {
                return;
            }
