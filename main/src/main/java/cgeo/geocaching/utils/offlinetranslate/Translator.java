...
public class Translator {
    ...
    private final Map<String, String> translationCache = Collections.synchronizedMap(new LeastRecentlyUsedMap.LruCache<>(500));
    ...
    private final AtomicInteger translateRunningCount = new AtomicInteger(0);
...
;
    ...
    private void retranslate(final boolean doReset, final String source, final BiConsumer<String, Boolean> translate) {
        synchronized (mutex) {
            ...
            translateRunningCount.addAndGet(1);
            Log.iForce(LOGPRAEFIX + " translate START (" + translateRunningCount.get() + "):" + source);
            changeState(TranslatorState.TRANSLATING);
            TranslatorUtils.translateAny(tImpl::translate, translationCache, source, translated -> {
                synchronized (mutex) {
                    Log.iForce(LOGPRAEFIX + " translate STOP (" + translateRunningCount.get() + ")" + source);
                    if (runId != translateRunId.get()) {
                        //run was abandoined
                        return;
                    }
                    final int cnt = translateRunningCount.decrementAndGet();
                    if (cnt == 0) {
