...
public final class GCLiveOAuth {
    ...
    private static final String TAG = "GCLiveOAuth";
    ...
    public static void startAuthorization(final Context context) {
        ...
        Log.i(TAG, "Stored code verifier, length=" + codeVerifier.length());
        ...
        Log.i(TAG, "Opening browser for authorization");
    ...
    public static boolean handleCallback(final Context context, final Uri uri) {
        ...
        return processOAuthCode(context, uri);
    }

    private static boolean isOAuthCallbackUri(@NonNull final Uri uri) {
        final String redirectUri = LocalizationUtils.getString(R.string.gc_live_redirect_uri);
        if (StringUtils.isBlank(redirectUri)) {
            return false;
        }
        final Uri redirect = Uri.parse(redirectUri);
        return StringUtils.equals(uri.getScheme(), redirect.getScheme())
                && StringUtils.equals(uri.getAuthority(), redirect.getAuthority())
                && StringUtils.equals(uri.getPath(), redirect.getPath());
    }

    private static boolean processOAuthCode(@NonNull final Context context, @NonNull final Uri uri) {
        final String code = uri.getQueryParameter("code");
        if (StringUtils.isBlank(code)) {
            final String error = uri.getQueryParameter("error");
            Log.e(TAG, "No code in callback: " + (error != null ? error : uri));
            showToast(context, LocalizationUtils.getString(R.string.live_api_auth_failed, error != null ? error : "no code"));
            return true;
        }
        final String codeVerifier = Settings.getGCLiveCodeVerifier();
        Log.i(TAG, "Code verifier from Settings: " + (codeVerifier != null ? "length=" + codeVerifier.length() : "NULL"));
        // Clear it immediately so it can't be reused
        Settings.setGCLiveCodeVerifier(null);
        if (StringUtils.isBlank(codeVerifier)) {
            Log.e(TAG, "No code verifier found in Settings");
            showToast(context, LocalizationUtils.getString(R.string.live_api_auth_failed_no_code_verifier));
            return true;
        }
        Log.i(TAG, "Got authorization code, exchanging for tokens...");
    ...
    private static void exchangeCodeForTokens(final Context context, final String code, final String codeVerifier) {
        try {
            ...
            Log.i(TAG, "POSTing to token endpoint");
            ...
            if (status != 200) {
                final String bodyStr = response.getBodyString();
                Log.e(TAG, "Token exchange HTTP " + status + ": " + bodyStr);
                showToast(context, LocalizationUtils.getString(R.string.live_api_auth_failed_http, status));
                return;
            }
            ...
            if (token == null || StringUtils.isBlank(token.accessToken)) {
                ...
                showToast(context, LocalizationUtils.getString(R.string.live_api_auth_failed_empty_token));
                return;
            }
            ...
            showToast(context, LocalizationUtils.getString(R.string.live_api_authorized));

        } catch (final Exception e) {
            Log.e(TAG, "Token exchange failed", e);
            showToast(context, LocalizationUtils.getString(R.string.live_api_auth_failed, e.getMessage()));
        }
    }
