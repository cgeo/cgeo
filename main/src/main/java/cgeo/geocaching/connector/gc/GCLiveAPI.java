...
public final class GCLiveAPI {
    ...
    private static String refreshAccessToken(final String refreshToken) {
        try {
            ...
            if (status != 200) {
                Log.e("GCLiveAPI: Token refresh HTTP " + status + ": " + response.getBodyString());
                return null;
            }
    ...
    static SearchResult searchByFilter(@NonNull final GeocacheFilter filter, @NonNull final GeocacheSort sort,
        ...
        try {
            ...
            final int totalCount = parseTotalCountHeader(response);
    ...
    private static int parseTotalCountHeader(@NonNull final HttpResponse response) {
        final String header = response.getResponse() != null ? response.getResponse().header("x-total-count") : null;
    ...
    private static String formatRange(@Nullable final Float min, @Nullable final Float max,
        ...
        final float from = min != null ? Math.round(Math.max(defaultMin, Math.min(defaultMax, min)) * 2f) / 2f : defaultMin;
        final float to = max != null ? Math.round(Math.max(defaultMin, Math.min(defaultMax, max)) * 2f) / 2f : defaultMax;
        return from + "-" + to;
    }
    ...
    private static final Map<GeocacheSort.SortType, String> SORT_TYPE_MAP;
    static {
        final Map<GeocacheSort.SortType, String> m = new EnumMap<>(GeocacheSort.SortType.class);
        m.put(GeocacheSort.SortType.NAME, "name");
        m.put(GeocacheSort.SortType.FAVORITES, "favoritepoints");
        m.put(GeocacheSort.SortType.FAVORITES_RATIO, "favoritepoints");
        m.put(GeocacheSort.SortType.DIFFICULTY, "difficulty");
        m.put(GeocacheSort.SortType.TERRAIN, "terrain");
        m.put(GeocacheSort.SortType.HIDDEN_DATE, "placeddate");
        m.put(GeocacheSort.SortType.LAST_FOUND, "founddate");
        SORT_TYPE_MAP = Collections.unmodifiableMap(m);
    }

    @NonNull
    private static String mapSortType(@NonNull final GeocacheSort.SortType sortType) {
        return SORT_TYPE_MAP.getOrDefault(sortType, "distance");
    }
    ...
    static SearchResult searchByViewport(@NonNull final Viewport viewport, @Nullable final GeocacheFilter filter) {
        ...
        try {
            final List<String> parts = buildViewportQueryParts(viewport, filter);
            ...
            final List<Geocache> allCaches = Collections.synchronizedList(new ArrayList<>());
            final List<io.reactivex.rxjava3.core.Observable<List<Geocache>>> observables = new ArrayList<>(numPages);

            for (int page = 0; page < numPages; page++) {
                final int skip = page * VIEWPORT_PAGE_SIZE;
                observables.add(io.reactivex.rxjava3.core.Observable.fromCallable(() -> {
                    final List<Geocache> pageCaches = new ArrayList<>();
                    try {
                        final HttpResponse response = apiReq()
                                .uri("/geocaches/search")
                        ...
                        if (status == 200) {
                            ...
                            if (apiCaches != null) {
                                for (final LiveApiGeocache apiCache : apiCaches) {
                                    ...
                                    pageCaches.add(cache);
                                }
                            }
                        } else if (status == 429) {
                            waitForRateLimit(response);
                            // skip this page on rate limit — better than blocking all threads
                        } else {
            ...
            // Wait for all pages to complete, collect results
            io.reactivex.rxjava3.core.Observable.merge(observables)
                    .blockingForEach(allCaches::addAll);
    ...
    @NonNull
    private static List<String> buildViewportQueryParts(@NonNull final Viewport viewport, @Nullable final GeocacheFilter filter) {
        final List<String> parts = new ArrayList<>();
    ...
    private static final Map<GeocacheFilterType, BiConsumer<BaseGeocacheFilter, List<String>>> FILTER_PARTS_HANDLERS;
    static {
        final Map<GeocacheFilterType, BiConsumer<BaseGeocacheFilter, List<String>>> m = new EnumMap<>(GeocacheFilterType.class);
        m.put(GeocacheFilterType.TYPE, (bf, p) -> addTypeFilterPart((TypeGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.SIZE, (bf, p) -> addSizeFilterPart((SizeGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.DIFFICULTY, (bf, p) -> addDifficultyFilterPart((DifficultyGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.TERRAIN, (bf, p) -> addTerrainFilterPart((TerrainGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.DIFFICULTY_TERRAIN, (bf, p) -> addDtFilterPart((DifficultyAndTerrainGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.NAME, (bf, p) -> addNameFilterPart((NameGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.OWNER, (bf, p) -> addOwnerFilterPart((OwnerGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.STATUS, (bf, p) -> addStatusFilterPart((StatusGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.HIDDEN, (bf, p) -> addHiddenDateFilterPart((HiddenGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.EVENT_DATE, (bf, p) -> addHiddenDateFilterPart((HiddenGeocacheFilter) bf, p));
        m.put(GeocacheFilterType.LOG_ENTRY, (bf, p) -> addLogEntryFilterPart((LogEntryGeocacheFilter) bf, p));
        FILTER_PARTS_HANDLERS = Collections.unmodifiableMap(m);
    }
    ...
    private static void addFilterToParts(@NonNull final BaseGeocacheFilter bf, @NonNull final List<String> parts) {
        final BiConsumer<BaseGeocacheFilter, List<String>> handler = FILTER_PARTS_HANDLERS.get(bf.getType());
        if (handler != null) {
            handler.accept(bf, parts);
        }
    }

    private static void addTypeFilterPart(@NonNull final TypeGeocacheFilter tf, @NonNull final List<String> parts) {
    ...
    private static void addSizeFilterPart(@NonNull final SizeGeocacheFilter sf, @NonNull final List<String> parts) {
        final List<String> ids = new ArrayList<>();
    ...
    private static void addDifficultyFilterPart(@NonNull final DifficultyGeocacheFilter df, @NonNull final List<String> parts) {
    ...
    private static void addTerrainFilterPart(@NonNull final TerrainGeocacheFilter tf, @NonNull final List<String> parts) {
    ...
    private static void addDtFilterPart(@NonNull final DifficultyAndTerrainGeocacheFilter dtf, @NonNull final List<String> parts) {
    ...
    private static void addNameFilterPart(@NonNull final NameGeocacheFilter nf, @NonNull final List<String> parts) {
    ...
    private static void addOwnerFilterPart(@NonNull final OwnerGeocacheFilter of, @NonNull final List<String> parts) {
    ...
    private static void addStatusFilterPart(@NonNull final StatusGeocacheFilter stf, @NonNull final List<String> parts) {
    ...
    private static void addHiddenDateFilterPart(@NonNull final HiddenGeocacheFilter hf, @NonNull final List<String> parts) {
    ...
    private static void addLogEntryFilterPart(@NonNull final LogEntryGeocacheFilter lf, @NonNull final List<String> parts) {
    ...
    static boolean uploadModifiedCoordinates(@NonNull final String geocode, @NonNull final Geopoint coords) {
        try {
            final String body = "{\"latitude\":" + coords.getLatitude() + ",\"longitude\":" + coords.getLongitude() + "}";
            final HttpResponse response = apiReq()
    ...
    static boolean uploadPersonalNote(@NonNull final String geocode, @NonNull final String note) {
        try {
            final String body = "{\"note\":\"" + escapeJson(note) + "\"}";
            final HttpResponse response = apiReq()
    ...
    public static Trackable searchTrackable(@Nullable final String geocode, @Nullable final String guid, @Nullable final String id) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(geocode) && StringUtils.startsWithIgnoreCase(geocode, "TB")) {
                response = fetchTrackableByCode(geocode);
            } else {
                response = fetchTrackableByNumber(geocode, id);
            }
            if (response == null) {
                return null;
            }
            ...
            final JsonNode tb = extractTrackableNode(json);
            if (tb == null) {
                return null;
            }
    ...
    @Nullable
    private static HttpResponse fetchTrackableByCode(@NonNull final String geocode) {
        try {
            return apiReq()
            ...
            Log.w("GCLiveAPI: fetchTrackableByCode failed", e);
            return null;
        }
    }

    @Nullable
    private static HttpResponse fetchTrackableByNumber(@Nullable final String geocode, @Nullable final String id) {
        ...
        try {
            ...
            Log.w("GCLiveAPI: fetchTrackableByNumber failed", e);
            return null;
        }
    }

    @Nullable
    private static JsonNode extractTrackableNode(@NonNull final JsonNode json) {
        if (json.isArray()) {
            return json.size() > 0 ? json.get(0) : null;
        }
        return json;
    }
    ...
    private static Trackable mapToTrackable(@NonNull final JsonNode tb) {
        ...
        applyTrackableHolder(trackable, tb);
        ...
        // Image
        applyTrackableImages(trackable, tb);

        return trackable;
    }

    private static void applyTrackableHolder(@NonNull final Trackable trackable, @NonNull final JsonNode tb) {
    ...
    private static void applyTrackableImages(@NonNull final Trackable trackable, @NonNull final JsonNode tb) {
    ...
    static Collection<ImmutablePair<Float, Float>> getNeededDifficultyTerrainCombisFor81Matrix() {
        try {
            ...
            final Set<String> found = buildFoundCombosSet(counts);
    ...
    @NonNull
    private static Set<String> buildFoundCombosSet(@NonNull final JsonNode counts) {
    ...
    public static SearchResult searchByBookmarkList(@NonNull final IConnector con, @NonNull final String listReferenceCode, final int alreadyTaken) {
        try {
            ...
            final int totalCount = totalHeader != null ? Integer.parseInt(totalHeader) : 0;

            final JsonNode json = JsonUtils.reader.readTree(response.getBodyString());
            final List<Geocache> caches = new ArrayList<>();
            for (final JsonNode node : json) {
                caches.add(mapBookmarkListItem(node));
            }
    ...
    @NonNull
    private static Geocache mapBookmarkListItem(@NonNull final JsonNode node) {
    ...
    static String createBookmarkList(@NonNull final String name) {
        try {
            final String body = "{\"name\":\"" + escapeJson(name) + "\",\"type\":{\"code\":\"bm\"}}";
            final HttpResponse response = apiReq()
                    .uri("/lists")
                    .method(HttpRequest.Method.POST)
                    .bodyJson(body)
    ...
    @NonNull
    private static String escapeJson(@NonNull final String s) {
        return s.replace("\\", "\\\\")
                .replace("\"", "\\\"")
                .replace("\n", "\\n")
                .replace("\r", "\\r")
    ...
    public static void fetchAndSaveLogs(@NonNull final String geocode) {
        for (int attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                final int status = response.getStatusCode();
                if (shouldRetryOnStatus(status, response)) {
                    continue;
                }
                if (status != 200) {
    ...
    private static boolean shouldRetryOnStatus(final int status, @NonNull final HttpResponse response) {
        if (status == 429) {
            waitForRateLimit(response);
            return true;
        }
    ...
    public static List<Geocache> fetchGeocachesBatch(@NonNull final List<String> geocodes) {
        ...
        for (int offset = 0; offset < geocodes.size(); offset += BATCH_SIZE) {
            final int end = Math.min(offset + BATCH_SIZE, geocodes.size());
    ...
    static Geocache mapToGeocache(@NonNull final LiveApiGeocache api) {
        ...
        // Owner
        applyOwnerInfo(cache, api);

        // Status
        applyCacheStatus(cache, api);

        // Coordinates
        applyCoordinates(cache, api);
        ...
        // User data
        applyUserData(cache, api);

        // Attributes
        applyAttributes(cache, api);

        // Additional waypoints
        applyWaypoints(cache, api);
    ...
    private static void applyOwnerInfo(@NonNull final Geocache cache, @NonNull final LiveApiGeocache api) {
        if (api.owner != null) {
    ...
    private static void applyCacheStatus(@NonNull final Geocache cache, @NonNull final LiveApiGeocache api) {
    ...
    private static void applyCoordinates(@NonNull final Geocache cache, @NonNull final LiveApiGeocache api) {
    ...
    private static void applyUserData(@NonNull final Geocache cache, @NonNull final LiveApiGeocache api) {
    ...
    private static void applyAttributes(@NonNull final Geocache cache, @NonNull final LiveApiGeocache api) {
        ...
        for (final LiveApiAttribute attr : api.attributes) {
            attrs.add(CacheAttribute.getById(attr.id).getValue(attr.isOn));
        }
        cache.setAttributes(attrs);
    }

    private static void applyWaypoints(@NonNull final Geocache cache, @NonNull final LiveApiGeocache api) {
