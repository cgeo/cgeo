...
public class MapDownloaderOpenAndroMaps extends AbstractMapDownloader {
    ...
    protected Download checkUpdateFor(final @NonNull String page, final String remoteUrl, final String remoteFilename) {
        ...
        while (matchMap.find()) {
            ...
            if (filename.equals(remoteFilenameNew)) {
                final long size = Long.parseLong(matchMap.group(4));
                if (size > 0) {
                    return new Download(matchMap.group(2), Uri.parse(getUpdatedPath(remoteUrl) + "/" + filename), false, CalendarUtils.yearMonthDay(CalendarUtils.parseDayMonthYearUS(matchMap.group(3))), Formatter.formatBytes(size), offlineMapType, iconRes);
                }
            }
        }
        return null;
    ...
    public DownloaderUtils.DownloadDescriptor getExtrafile(final Activity activity, final Uri mapUri) {
        return getExtrafile(THEME_FILES, LocalizationUtils.getString(R.string.mapserver_openandromaps_themes_downloadurl), Download.DownloadType.DOWNLOADTYPE_THEME_OPENANDROMAPS);
    }
