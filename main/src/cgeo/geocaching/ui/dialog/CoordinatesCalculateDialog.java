...
public class CoordinatesCalculateDialog extends DialogFragment implements ClickCompleteCallback, LongClickCompleteCallback {
    ...
    private String addLeadingZerosToDecimal(final String coordinate, boolean lat) {
    ...
    private Spannable formatCoordinateString(String coordinateString, int leadingZeros, int paddingColour)
    {
        Spannable returnValue = new SpannableString(coordinateString);

        if (leadingZeros > 0) {
            Pattern pat = Pattern.compile("[\\.,]");
            Matcher match = pat.matcher(coordinateString);
            if (match.find()) {
                int point = match.start() + 1;
