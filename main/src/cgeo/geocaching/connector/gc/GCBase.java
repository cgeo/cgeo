...
public class GCBase {
    ...
    private final static LeastRecentlyUsedCache<Integer, Tile> tileCache = new LeastRecentlyUsedCache<Integer, Tile>(32);
        ...
        if (null != coords) {
            ...
            for (Tile tile : tiles) {
                if (tile.containsPoint(coords)) {
                    ...
                    Log.d(Settings.tag, "Removing " + tile.toString());
                }
            }
    ...
    private static SearchResult searchByViewport(final Viewport viewport, final String[] tokens, Strategy strategy) {
        ...
        if (strategy.flags.contains(StrategyFlag.SEARCH_NEARBY)) {
            ...
            if ((null == lastSearchViewport) || !lastSearchViewport.isInViewport(center)) {
