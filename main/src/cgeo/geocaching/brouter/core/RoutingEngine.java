...
public class RoutingEngine extends Thread {
    ...
    private OsmTrack findTrackHelper(final String operationName, final MatchedWaypoint startWp, final MatchedWaypoint endWp, final OsmTrack costCuttingTrack, final OsmTrack refTrack, final boolean fastPartialRecalcIn) {
        boolean fastPartialRecalc = fastPartialRecalcIn;
    ...
    public int[] getOpenSet() {
        ...
        synchronized (openSet) {
            if (guideTrack != null) {
                final ArrayList<OsmPathElement> nodes = guideTrack.nodes;
                final int[] res = new int[nodes.size() * 2];
                int i = 0;
            ...
            final int size = openSet.getExtract(extract);
            final int[] res = new int[size * 2];
            for (int i = 0, j = 0; i < size; i++) {
                final OsmPath p = (OsmPath) extract[i];
                extract[i] = null;
                final OsmNode n = p.getTargetNode();
                res[j++] = n.ilon;
                res[j++] = n.ilat;
            }
            return res;
