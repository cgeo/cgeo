...
public class TinyDenseLongMap extends DenseLongMap {
    ...
    private void fillReturnValue(final byte[] rv, final int idx, final int p) {
        rv[0] = vla[idx][p];
        if (rv.length == 2) {
            vla[idx][p] = rv[1];
        }
    ...
    @Override
    public void put(final long id, final int value) {
        final byte[] rv = new byte[2];
        rv[1] = (byte) value;
        if (contains(id, rv)) {
            return;
        }
     ...
     */
    @Override
    public int getInt(final long id) {
    ...
    private boolean addHelper(final long id) {
    ...
    private boolean contains(final long id, final byte[] rv) {
        // determine the first empty array
        int bp = size; // treat size as bitpattern
        int idx = 1;

        while (bp != 0) {
            if ((bp & 1) == 1 && contains(idx, id, rv)) { // array at idx is valid, check
                return true;
            }
            idx++;
            bp >>= 1;
    ...
    private boolean contains(final int idx, final long id, final byte[] rv) {
        final long[] a = al[idx];
        int offset = a.length;
        int n = 0;
